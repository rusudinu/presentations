...
\begin{frame}[eyebrow=Serverless]{Serverless: functions as a service}
  ...
  \end{takeaway}
  \note{Ask who has already used a FaaS platform without knowing the term. Most students have, through Firebase Cloud Functions or a hosting provider's edge functions. The unfamiliar part is usually the billing model, not the code.}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.58\textwidth}
      \begin{itemize}
        ...
        \item Go's log/slog, in the standard library, writes JSON by default: a level, a message, and any fields attached, one line per event.
