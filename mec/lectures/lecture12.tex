...
\end{dart}
    ...
    \begin{column}{0.41\textwidth}
      \lead{The channel name is just a string.}{Both sides must agree on it byte for byte. A typo compiles fine and fails at run time.}
      \muted{\code{PlatformException} carries a code the native side chose. \code{MissingPluginException} means nothing answered on this platform.}
    \end{column}
  \end{columns}
...
actual class KeyValueStore
  ...
  val p = ctx.getSharedPreferences(name, 0)
  actual fun put(k: String, v: String) =
...
actual class KeyValueStore
    actual constructor(name: String) {
  val d = NSUserDefaults(suiteName = name)
  actual fun put(k: String, v: String) =
