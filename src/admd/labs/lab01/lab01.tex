...
\begin{frame}[eyebrow=Setup]{The cumulative lab app}
  \begin{steps}
    ...
    \item One branch per lab. One pull request per lab, merged into \code{main}.
...
\begin{frame}[eyebrow=GitHub]{The pull request workflow}
  \begin{steps}
    ...
    \item Merge it, so the next lab starts from \code{main}.
  \end{steps}
\end{frame}

\begin{frame}[fragile,eyebrow=GitHub]{First pull request}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
\begin{shell}
git add .
...
\closingframe{Submission}{Push to branch \code{lab-1} with a screenshot and one commit}
