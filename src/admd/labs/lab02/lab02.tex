...
\begin{frame}[eyebrow=Today]{Lab objectives}
  \vskip 2mm
  \begin{grid}[2]
    \cell[\faLock]{Null safety in code}{When to use \code{?}, \code{?.}, \code{??} and \code{late}}
...
\begin{frame}[eyebrow=Reference]{Null-safety types and operators}
  {\small
  \begin{columns}[T]
  ...
  \vskip 2mm
  \begin{takeaway}{Prefer \code{?.}, \code{??} and null checks over \code{!}.}
    The compiler cannot check a value asserted with \code{!}.
  \end{takeaway}
\end{frame}

\begin{frame}[fragile,eyebrow=Reference]{Null safety example}
\begin{dart}
class Room {
...
\begin{frame}[fragile,eyebrow=Scaffolding]{Minimal UI for the models}
\begin{dart}
ListView.builder(
...
\closingframe{Submission}{Branch lab-2 · one commit per task}
