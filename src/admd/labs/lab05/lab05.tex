...
\decklabel{Lab 5}
\title{BLoC}
\subtitle{Cubit, Bloc events and bloc\_test}
...
\begin{frame}[fragile,eyebrow=Setup]{Feature folder structure}
  \begin{codeblock}
lib/
  models/todo.dart
  todos/
...
\begin{frame}[fragile,eyebrow=Task I]{TodoCubit methods}
\begin{dart}
...
\begin{frame}[fragile,eyebrow=Task I]{BlocProvider and BlocBuilder}
\begin{dart}
...
\begin{frame}[eyebrow=Reference]{From Cubit to Bloc}
  \vskip 2mm
  \begin{center}
  \begin{tabular}{lll}
    \toprule
...
\begin{frame}[fragile,eyebrow=Task II]{TodoRepository and dio}
\begin{dart}
...
\begin{frame}[fragile,eyebrow=Task II]{TodoBloc event handlers}
\begin{dart}
...
\begin{frame}[eyebrow=Task III]{Testing with bloc\_test}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{steps}
...
\begin{frame}[fragile,eyebrow=Task III]{Test with two emitted states}
\begin{dart}
void main() {
...
\begin{frame}[eyebrow=Troubleshooting]{Common errors and fixes}
  \vskip 2mm
...
\closingframe{Submission}{Branch lab-5 · TodoBloc replaces TodoCubit · one passing bloc\_test in the PR}
