...
\begin{frame}[fragile,eyebrow=Setup]{Setup}
  \begin{steps}
    \item Branch from the Lab 5 state: \code{git checkout -b lab-6}
    \item Add \code{firebase\_core}, \code{firebase\_auth}, \code{google\_sign\_in}, \code{go\_router} and \code{web\_socket\_channel}.
    \item Create a Firebase project, run \code{flutterfire configure}, initialize Firebase in \code{main}.
    \item Register the debug \emph{and} release SHA-1 fingerprints in Firebase, or Google sign-in fails silently.
  \end{steps}
  \begin{hint}
\begin{shell}
...
\begin{frame}[fragile,eyebrow=Task I]{Auth state with authStateChanges}
\begin{dart}
StreamBuilder<User?>(
...
\end{dart}
    ...
    \begin{column}{0.36\textwidth}
      \begin{steps}
        ...
        \item Copy \code{GoRouterRefreshStream} from the go\_router example. It is not exported.
      \end{steps}
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}[eyebrow=Task II]{Router guard and refreshListenable}
  \begin{grid}[2]
    ...
    \cell[\faIcon{sync}]{With it}{Every auth change re-runs \code{redirect} immediately, including the first frame and any deep link.}
  \end{grid}
  \vskip 6mm
  \kv{Done when}{}
  \begin{checklist}
    \item Signing out from Home returns to \code{/login} without a manual navigation call.
    \item Opening \code{admd://app/live} while signed out lands on \code{/login}, not on \code{LiveScreen}.
    \item Signing in from \code{/login} lands back on \code{/}, never on \code{/login} itself.
  \end{checklist}
\end{frame}

\begin{frame}[fragile,eyebrow=Task III]{WebSocket connection}
\begin{dart}
...
\begin{frame}[fragile,eyebrow=Reference]{Echo server source}
\begin{golang}
...
\begin{frame}[fragile,eyebrow=Reference]{Local echo server}
  \begin{grid}[2]
    ...
    \cell[\faIcon{vial}]{You can see both sides}{The terminal running \code{go run .} prints every frame.}
  \end{grid}
  \vskip 4mm
\begin{shell}
go mod init echo
...
\closingframe{Submission}{Branch lab-6 · one commit per task · echo server included in the repo}
