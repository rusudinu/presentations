...
\decklabel{Lab 7}
\title{Lab test}
\subtitle{Rules, format, practice specs}
...
\begin{frame}[eyebrow=Today]{Lab objectives}
  ...
  \begin{grid}[2]
    ...
    \cell[\faIcon{graduation-cap}]{Worth 30 points}{7.5 points per requirement, graded from your pushed branch. There is no live demo.}
  \end{grid}
  \vskip 6mm
  \taskmeta{Time}{90 minutes}{Submit}{Push to admd-labs, branch lab-7, before time is up}
\end{frame}

\begin{frame}[eyebrow=Rules]{Rules}
  \begin{steps}
  ...
  \end{steps}
  \begin{takeaway}{Time lost to a crashed emulator or a broken hot reload is not added back.}
  \end{takeaway}
\end{frame}

\begin{frame}[eyebrow=Rules]{Allowed resources}
  \begin{steps}
...
\begin{frame}[eyebrow=Format]{Test format}
  \vskip 2mm
  \begin{grid}[2]
...
\begin{frame}[eyebrow=Format]{Requirement types}
  \begin{grid}[2]
...
\begin{frame}[eyebrow=Rubric]{Rubric}
  {\usebeamerfont{small}\begin{tabular}{@{}p{24mm}p{42mm}p{42mm}@{}}
    \toprule
    \thead{Requirement} & \thead{Full credit (7.5 p)} & \thead{Partial credit (3.75 p)} \\
    \midrule
...
\begin{frame}[fragile,eyebrow=Submission]{Submission}
\begin{shell}
...
\end{shell}
  \begin{steps}
    ...
    \item Open one pull request titled with your name.
  \end{steps}
  \begin{warning}
...
\begin{frame}[eyebrow=Prepare]{Preparation}
  \vskip 2mm
  ...
  \end{tabular}}
  \vskip 4mm
  \begin{takeaway}{Every requirement was covered in Labs 3 to 6.}
...
\begin{frame}[eyebrow=Troubleshooting]{Common mistakes}
  \vskip 2mm
  ...
  \trouble{A blank screen, nothing in the console}{The Cubit never emitted after the network call finished. Check that the future completes.}
...
\closingframe{Submission}{Branch lab-7 · one pull request · nothing after the deadline}
