...
\subtitle{Course organization, Git and GitHub, app development strategies, Flutter setup}
...
\objectivesframe{
  \cell[\faIcon{calendar-alt}]{Course organization}{The fourteen lectures, the seven labs, how the grade is built, and what to bring to Lab 1.}
  \cell[\faIcon{code-branch}]{Git setup}{A GitHub account and the loop you run every lab: branch, commit, push, pull request, review, merge.}
  ...
  \cell[\faIcon{mobile-alt}]{Choosing a strategy}{Why this course uses Flutter, what that choice costs, and when another strategy fits better.}
}

\divider{Part 1 · Orientation}{Course organization}[Instructors, lectures, labs, grading]

\begin{frame}[eyebrow=Orientation]{Instructors}
  \vskip 2mm
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      ...
      \muted{rusudinu.com}
      \vskip 4mm
      \muted{I build Flutter apps professionally. What you see in this course is the toolchain I use at work.}
    \end{column}
    ...
    \end{column}
  \end{columns}
  \begin{takeaway}{Ask questions during the lecture.}
  \end{takeaway}
...
\begin{frame}[eyebrow=Orientation]{Course scope and the follow-up course}
  \vskip 2mm
  \begin{grid}[2]
  ...
  \end{grid}
  \begin{takeaway}{Topics that belong to the second course are only named here.}
  \end{takeaway}
...
\begin{frame}[eyebrow=Orientation]{Lecture schedule}
  \vskip 2mm
  {\usebeamerfont{small}
  \begin{columns}[T]
...
\begin{frame}[eyebrow=Orientation]{Lab schedule}
  \vskip 2mm
...
\begin{frame}[eyebrow=Orientation]{Grading}
  \vskip 4mm
  \begin{grid}[4]
    \bignum{20 p}{colloquium}
    \bignum{20 p}{midterm exam}
    \bignum{30 p}{lab assignments}
    \bignum{30 p}{practical lab test in Lab 7, individual}
  \end{grid}
  \vskip 4mm
  \begin{takeaway}{You pass with at least 50 points out of 100.}
  \end{takeaway}
  \note{Say the pass line out loud and repeat it. The message students need is that lab work is most of the grade, so falling behind in the labs cannot be recovered later. Mention that the lab test in week 14 is individual and practical, and that the colloquium and the midterm are separate from it.}
\end{frame}

\begin{frame}[eyebrow=Orientation]{Lab repository and branches}
  \vskip 2mm
  \begin{grid}[3]
    \cell[\faIcon{folder-open}]{One repository}{Each student keeps one GitHub repository named \code{admd-labs}. Every lab lands in it.}
    ...
    \cell[\faIcon{check-double}]{One pull request}{Each lab ends with a pull request, merged into \code{main} so the next lab starts from it.}
  \end{grid}
  \begin{takeaway}{Every lab lives in the repository.}
  \end{takeaway}
...
\divider{Part 2 · Git and GitHub}{Git workflow}[Git hosting, branches, commits, pull requests, continuous integration]

\begin{frame}[eyebrow=Git]{Git and Git hosting services}
  \vskip 2mm
  \begin{columns}[T]
...
\begin{frame}[eyebrow=Git]{GitHub account}
  \vskip 2mm
  \begin{columns}[T]
...
\begin{frame}[eyebrow=Git]{Repositories and branches}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.39\textwidth}
      \kv{In this course}{\code{main} is the finished state of your app. \code{lab-3} is the work in progress for Lab 3.}
      \vskip 3mm
...
\begin{frame}[fragile,eyebrow=Git]{Git workflow commands}
  \begin{columns}[T]
...
\end{shell}
    ...
    \begin{column}{0.40\textwidth}
      \lead{Run this loop once per lab.}{Clone, branch, commit, push, review, merge, pull.}
      \vskip 2mm
...
\begin{frame}[fragile,eyebrow=Git]{Commit messages}
  \begin{columns}[T]
...
\end{codeblock}
  ...
  \end{takeaway}
  \note{Ask the room what the bottom three messages tell a reviewer. Nothing. Mention that the imperative mood matches what Git itself writes for merges and reverts.}
\end{frame}
...
\begin{frame}[eyebrow=Git]{Continuous integration and delivery}
  \vskip 3mm
  \begin{grid}[3]
  ...
  \begin{takeaway}{A clean machine shows whether the repository holds everything the project needs.}
  \end{takeaway}
  \note{Keep this short: the full pipeline comes back in the release lecture. The idea students need now is that a pull request runs checks automatically, and a red check is not someone else's problem.}
\end{frame}

\divider{Part 3 · Strategies}{App development strategies}[Native, web, cross-platform, shared logic]

\begin{frame}[eyebrow=Strategies]{Native apps}
  \vskip 2mm
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{itemize}
...
\begin{frame}[eyebrow=Strategies]{Strategy 2: React Native}
  \vskip 2mm
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      \begin{itemize}
        \item The user interface is native, so each platform keeps its own look and behavior.
  ...
  \end{takeaway}
  \note{The old React Native architecture serialized messages over an asynchronous bridge. It is not only off by default, it has been removed from current releases, so do not repeat it. Note that a team already writing React on the web is the strongest reason to pick this option.}
\end{frame}

\begin{frame}[eyebrow=Strategies]{Strategy 2: Flutter}
  \vskip 2mm
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
...
\begin{frame}[eyebrow=Strategies]{Strategy 3: Kotlin Multiplatform}
  \vskip 2mm
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
...
\begin{frame}[eyebrow=Strategies]{Strategy comparison}
...
\begin{frame}[eyebrow=Strategies]{Choosing a strategy}
  \vskip 3mm
  \begin{tabular}{@{}>{\raggedright\arraybackslash}p{86mm}>{\raggedright\arraybackslash}p{46mm}@{}}
    ...
    \bottomrule
  \end{tabular}
  \begin{takeaway}{Team skills, deadline and hardware access usually pick the strategy.}
  \end{takeaway}
...
\divider{Part 4 · Setup}{Flutter setup}[Reasons for Flutter, trade-offs, installation]

\begin{frame}[eyebrow=Flutter]{Reasons for choosing Flutter}
  \vskip 3mm
  \begin{grid}[2]
...
\begin{frame}[eyebrow=Flutter]{Flutter trade-offs}
  \vskip 2mm
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{itemize}
...
\begin{frame}[eyebrow=Setup]{Required software}
  \vskip 2mm
  \begin{grid}[2]
...
\begin{frame}[fragile,eyebrow=Setup]{Installation check and course resources}
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
\begin{shell}
flutter doctor
...
\begin{frame}[eyebrow=Recap]{Summary}
  \vskip 2mm
  \begin{enumerate}
    \item Every lab lives in one repository. \muted{One repository, a branch per lab, a pull request per lab.}
