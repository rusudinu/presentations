...
\subtitle{Null safety, Dart language basics, Flutter overview, Flutter toolchain}
...
\objectivesframe{
  \cell[\faLock]{Null safety}{Read \code{?}, \code{??}, \code{?.}, \code{!} and \code{late}, and know why the compiler can trust them.}
  \cell[\faCode]{Dart basics}{Variables, functions, classes, collections, records and pattern matching.}
  \cell[\faIcon{mobile-alt}]{Flutter overview}{One Dart codebase, one renderer, and the widget catalog at a glance.}
  \cell[\faBolt]{Flutter toolchain}{Install, \code{flutter doctor}, hot reload versus hot restart, and a new project's layout.}
}

\divider{Part 1 · Null safety}{Null safety}[Nullable types, operators, late and required, Kotlin comparison]

\begin{frame}[fragile,eyebrow=Null safety]{Null reference errors}
  \begin{columns}[T]
...
\end{dart}
    ...
    \end{column}
  \end{columns}
  \begin{takeaway}{The compiler can only help if the type says a value may be absent.}
  \end{takeaway}
...
\begin{frame}[eyebrow=Null safety]{Limits of type safety}
  \vskip 2mm
  \begin{grid}[3]
  ...
  \end{grid}
  \vskip 4mm
  \begin{takeaway}{The one overlap is memory safety.}
...
\begin{frame}[fragile,eyebrow=Null safety]{Sound null safety}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
\begin{dart}
...
\end{dart}
    \end{column}
    \begin{column}{0.40\textwidth}
      \lead{Sound means the guarantee holds}{A non-nullable Dart variable can never hold null at run time.}
      \vskip 2mm
      \muted{So the compiler stops emitting null checks for it. Sound null safety also makes release builds smaller and faster.}
    \end{column}
  \end{columns}
...
\begin{frame}[fragile,eyebrow=Null safety]{The late and required keywords}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
\begin{dart}
...
\begin{frame}[fragile,eyebrow=Null safety]{Null safety in Dart and Kotlin}
  \begin{columns}[T]
...
\end{kotlin}
  ...
  \end{columns}
  \vskip 3mm
  \muted{Operators map almost one to one: \code{?.}, \code{??} (Kotlin: \code{?:}), \code{!} (Kotlin: \code{!!}), \code{late final} (Kotlin: \code{lateinit var}, \code{var} only).}
...
\divider{Part 2 · Dart}{Dart language basics}[Variables, functions, classes, collections, records, pattern matching]
...
\end{dart}
    \end{column}
    \begin{column}{0.40\textwidth}
      \lead{Why named parameters matter}{Every Flutter widget constructor uses them.}
      \vskip 2mm
      \muted{Functions are first-class values: pass them, store them, return them. That is what a callback like \code{onPressed} is.}
    \end{column}
  \end{columns}
...
\end{dart}
    ...
    \begin{column}{0.40\textwidth}
      ...
      \vskip 2mm
      \muted{\code{for}, \code{if} and \code{while} read like Kotlin and Java.}
    \end{column}
  \end{columns}
...
\begin{frame}[fragile,eyebrow=Dart]{Asynchronous code preview}
...
\begin{frame}[fragile,eyebrow=Dart]{Dart syntax reference}
  \vskip 2mm
...
\begin{frame}[fragile,eyebrow=Dart]{Kotlin and Dart compared}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      {\bfseries Dart}
\begin{dart}
...
\divider{Part 3 · Flutter}{Flutter overview}[Rendering, widget catalog, JIT and AOT, hot reload, toolchain]

\begin{frame}[eyebrow=Flutter]{Flutter architecture}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      \begin{itemize}
...
\begin{frame}[eyebrow=Flutter]{Widget catalog}
  \vskip 2mm
  \begin{grid}[3]
...
\begin{frame}[fragile,eyebrow=Flutter]{JIT and AOT compilation}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
\begin{panel}
...
\end{shell}
    \end{column}
    \begin{column}{0.40\textwidth}
      \lead{Run it first, always}{\code{flutter doctor} names the missing piece. Read that line before asking for help.}
      \vskip 2mm
...
\begin{frame}[fragile,eyebrow=Flutter]{Creating and running a new app}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
\begin{shell}
...
\begin{frame}[eyebrow=Flutter]{Key numbers}
  \vskip 4mm
  \begin{grid}[3]
