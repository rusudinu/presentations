...
\end{dart}
    ...
    \end{column}
  \end{columns}
  \note{This is the shape every model class in the labs will follow: a handful of final fields, one constructor. Point out room's ? makes it genuinely optional to omit.}
\end{frame}
...
class Lecture {
  ...
  // Named constructor: another way
  // to build the same type.
...
\begin{frame}[eyebrow=Flutter]{What Flutter is, and what it draws}
  \begin{columns}[T]
    ...
    \begin{column}{0.40\textwidth}
      ...
      \kv{GPU}{Metal on iOS, Vulkan on Android}
    \end{column}
  \end{columns}
