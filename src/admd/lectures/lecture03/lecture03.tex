...
\objectivesframe{
  \cell[\faIcon{layer-group}]{Widget trees}{Explain what a widget is, why it is immutable, and which of the three trees holds state.}
  \cell[\faIcon{ruler-combined}]{Layout}{Use Container, Padding, Row, Column, Expanded and Stack, and read the constraint rules.}
  \cell[\faIcon{list-ul}]{Scrolling lists}{Choose between ListView, ListView.builder, GridView and SingleChildScrollView.}
  \cell[\faIcon{mobile-alt}]{Material screens}{Wrap content in a Scaffold with an AppBar, a floating action button, and a SnackBar.}
}

\divider{Part 1 · Widgets}{Widgets}[Immutability, the three trees, composition, rebuilds]

\begin{frame}[eyebrow=Widgets]{Widgets}
  \vskip 2mm
  \begin{grid}[3]
...
\begin{frame}[fragile,eyebrow=Widgets]{Widget immutability}
  \begin{columns}[T]
...
\end{dart}
    ...
    \begin{column}{0.42\textwidth}
      ...
      \muted{Write \code{const} wherever the analyzer allows it.}
    \end{column}
  \end{columns}
...
\begin{frame}[fragile,eyebrow=Widgets]{Composition over inheritance}
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
\begin{dart}
...
\end{dart}
    \end{column}
    \begin{column}{0.42\textwidth}
      \lead{One widget, one job.}{Layout comes from nesting small widgets.}
...
\begin{frame}[fragile,eyebrow=Widgets]{StatelessWidget}
  \begin{columns}[T]
...
\begin{frame}[eyebrow=Widgets]{Rebuild scope and keys}
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
...
\begin{frame}[fragile,eyebrow=Widgets]{Extracting widgets}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
\begin{dart}
...
\divider{Part 2 · Layout}{Layout}[Constraints, Container, Row, Column, Expanded, Stack, overflow errors]

\begin{frame}[eyebrow=Layout]{Layout constraints}
  \vskip 3mm
  \begin{grid}[3]
...
\end{dart}
    ...
    \end{column}
  \end{columns}
  \note{Draw the two axes on the board for a Row and then rotate them for a Column. The single most common confusion is main versus cross when the axis changes. Dart's dot shorthands also accept mainAxisAlignment: .center, so expect to see both forms in code you read.}
\end{frame}
...
\begin{frame}[eyebrow=Layout]{Common layout widgets}
  \vskip 2mm
  ...
  \end{tabular}}
  \vskip 4mm
  \muted{Keep this table open during the lab.}
...
\begin{frame}[fragile,eyebrow=Layout]{RenderFlex overflow errors}
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
\begin{dart}
...
\end{dart}
    ...
    \begin{column}{0.42\textwidth}
      ...
      \muted{For text, \code{maxLines} with an ellipsis overflow is often what you want.}
    \end{column}
  \end{columns}
...
\divider{Part 3 · Scrolling}{Scrolling}[ListView, builders, grids, scroll views]

\begin{frame}[fragile,eyebrow=Scrolling]{ListView and ListView.builder}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
\begin{dart}
ListView(   // eager: every child built now
  children: [
    for (final t in todos) TodoTile(todo: t),
  ],
)

ListView.builder(   // lazy: visible slice
...
\divider{Part 4 · Material}{Material components}[Scaffold, AppBar, buttons, SnackBars and dialogs]

\begin{frame}[fragile,eyebrow=Material]{Scaffold}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
\begin{dart}
Scaffold(
...
\begin{frame}[eyebrow=Material]{Cupertino and adaptive widgets}
  ...
  \begin{grid}[3]
    ...
    \cell[\faIcon{mobile-alt}]{Pick one and commit}{Most teams ship Material on both platforms.}
  \end{grid}
  \begin{takeaway}{This course uses Material.}
    Page transitions already adapt. The labs are graded on Material widgets.
  \end{takeaway}
  \note{Ask who has an iPhone and who has an Android phone, then ask whether they would notice a Material app on iOS. Most would not. Say that Cupertino matters when the client asks for it, not by default. Recent Flutter releases also ship Material and Cupertino as standalone packages, material\_ui and cupertino\_ui; the widgets are the same and the flutter/material.dart import still works.}
\end{frame}

\divider{Part 5 · Worked example}{Lab 3 todo screen}[Screen state, list, TodoTile, adding and removing items, empty state]
...
\end{dart}
    ...
    \end{column}
  \end{columns}
  \note{Say out loud that a mutable done field is fine today and will be replaced by an immutable model with copyWith in Lecture 6. Building it the simple way first makes the later change meaningful.}
\end{frame}
...
\end{dart}
    \end{column}
    \begin{column}{0.40\textwidth}
      \lead{Empty, loading and error states are part of the design.}{}
