...
\subtitle{Futures, Streams, heavy computation, agent-assisted coding}
...
\objectivesframe{
  \cell[\faBolt]{Futures and async/await}{Futures, async/await, and try/catch/finally around every network or database call.}
  ...
  \cell[\faIcon{tachometer-alt}]{Heavy computation}{Recognize CPU-bound code that freezes the UI, and move it off the main thread with compute().}
  \cell[\faIcon{robot}]{Agent-assisted coding}{Prompt it with real context, then review its output like any other pull request.}
}
...
\divider{Part 1 · Async Dart}{Asynchronous Dart}[Futures, await, and error handling]

\begin{frame}[eyebrow=Async]{The UI thread}
  \vskip 2mm
  \begin{itemize}
  ...
  \end{itemize}
  \begin{takeaway}{The thread blocks until your code yields control back to it.}
  \end{takeaway}
...
\begin{frame}[fragile,eyebrow=Async]{Futures}
  \begin{columns}[T]
...
\begin{frame}[fragile,eyebrow=Async]{async and await}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
\begin{dart}
...
\begin{frame}[fragile,eyebrow=Async]{Sequential and concurrent awaits}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
\begin{dart}
...
\begin{frame}[eyebrow=Async]{Errors in awaited calls}
  \vskip 2mm
  \begin{itemize}
    \item A failure inside async code does not throw at the call site. It completes the Future with an error instead.
    \item await rethrows that error at the point of the await, so try, catch and finally behave as they do in synchronous code.
...
\divider{Part 2 · Streams and heavier work}{Streams and heavy computation}[Streams, FutureBuilder, StreamBuilder, and compute()]

\begin{frame}[fragile,eyebrow=Streams]{Streams}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
\begin{dart}
...
\begin{frame}[eyebrow=Streams]{Sources of Futures and Streams}
  \vskip 2mm
  \begin{itemize}
...
\begin{frame}[fragile,eyebrow=Streams]{FutureBuilder}
\begin{dart}
FutureBuilder<User>(
  future: _userFuture,
...
\begin{frame}[fragile,eyebrow=Streams]{Creating the Future outside build}
  \begin{columns}[T]
...
\begin{frame}[fragile,eyebrow=Streams]{StreamBuilder}
\begin{dart}
StreamBuilder<int>(
  stream: ticker(),
  initialData: 0,
...
\begin{frame}[eyebrow=Streams]{Combining a Future and a Stream}
  \vskip 2mm
  \begin{itemize}
  ...
  \end{itemize}
  \begin{takeaway}{Most screens need both.}
...
\begin{frame}[eyebrow=Streams]{FutureBuilder vs StreamBuilder}
  \vskip 1mm
...
\begin{frame}[fragile,eyebrow=Streams]{Asynchrony and parallelism}
\begin{dart}
...
\begin{frame}[fragile,eyebrow=Streams]{compute() for heavy work}
\begin{dart}
...
\begin{frame}[eyebrow=Streams]{Choosing between await and compute()}
  \vskip 2mm
  \begin{itemize}
...
\divider{Part 3 · Agent-assisted coding}{Agent-assisted coding}[Tools, MCP, prompting, code review, lab rules]

\begin{frame}[eyebrow=Agents]{AI coding tools in 2026}
  \vskip 2mm
  \begin{grid}[3]
...
\begin{frame}[eyebrow=Agents]{Model Context Protocol (MCP)}
  \vskip 2mm
  \begin{itemize}
...
\begin{frame}[eyebrow=Agents]{Agent strengths}
  \vskip 2mm
  \begin{itemize}
...
\begin{frame}[eyebrow=Agents]{Agent weaknesses}
  \vskip 2mm
  \begin{itemize}
    ...
    \item Silent drift away from the package versions you are on.
...
\begin{frame}[eyebrow=Agents]{Common agent mistakes in Flutter}
  \vskip 2mm
  \begin{itemize}
  ...
  \end{itemize}
  \begin{takeaway}{Read the diff as you would a stranger's pull request.}
  \end{takeaway}
...
\begin{frame}[fragile,eyebrow=Agents]{Prompting with context}
  \begin{columns}[T]
...
\begin{frame}[eyebrow=Agents]{Reviewing agent-generated code}
  \vskip 2mm
  \begin{grid}[3]
  ...
  \end{grid}
  \note{Tie this back to Lecture 1: every lab runs on the branch, pull request, review loop. Agent-generated code makes review more important, not less, because the volume of code goes up.}
\end{frame}

\begin{frame}[eyebrow=Agents]{Rules for agent use in the labs}
  \vskip 1mm
...
\begin{frame}[fragile,eyebrow=Agents]{Citing an agent in a pull request}
\begin{codeblock}
## What changed
