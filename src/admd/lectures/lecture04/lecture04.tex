...
\end{dart}
    \end{column}
    \begin{column}{0.40\textwidth}
      \lead{One specific catch, one general catch.}{Handle the failure you expect by type, then anything else.}
      \muted{Always check \code{mounted} before calling setState after an await. The widget may already have left the tree.}
    \end{column}
  \end{columns}
...
\begin{frame}[eyebrow=Agents]{MCP: how an agent reaches your tools}
  ...
  \end{takeaway}
  \note{Keep this concrete: an agentic CLI in the lab can genuinely run git commands and delete files if asked to, which is why the review habit on the next slides is not optional.}
\end{frame}
...
\begin{frame}[eyebrow=Recap]{Three things to remember}
  ...
  \end{enumerate}
  \vskip 5mm
  \kv{Further reading}{\link{https://dart.dev/language/async}{dart.dev/language/async} · \link{https://docs.flutter.dev/cookbook/networking/fetch-data}{FutureBuilder cookbook}}
