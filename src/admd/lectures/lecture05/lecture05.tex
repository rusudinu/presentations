...
\begin{frame}[eyebrow=Debugging]{What each DevTools tab is for}
  \begin{grid}[3]
    \cell[\faLayerGroup]{Inspector}{The live widget tree and the Layout Explorer for overflow.}
    \cell[\faIcon{tachometer-alt}]{Performance}{Frame timeline, UI versus raster thread, which frames missed budget.}
...
\begin{frame}[eyebrow=Debugging]{The Widget Inspector: selecting and layout}
  ...
  \begin{itemize}
    ...
    \item The Layout Explorer shows flex factors and incoming constraints. It is where you find the cause of a RenderFlex overflow, the yellow-and-black stripes.
  \end{itemize}
  \vskip 3mm
  \begin{takeaway}{You cannot reason about layout from the source alone.}
    The Layout Explorer shows the constraints your widget actually received, not the ones you assumed.
  \end{takeaway}
...
\begin{frame}[eyebrow=Debugging]{The Widget Inspector: rebuilds and repaints}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      \lead{Track widget rebuilds}{Counts how many times each widget rebuilt. That count is the measurement this lecture is about.}
      \muted{Highlight repaints wraps every layer in a rotating color. Anything flashing constantly is repainting when it should not.}
    \end{column}
    \begin{column}{0.40\textwidth}
      \begin{hint}[Try this in the lab]
        \begin{steps}
          \item Run your lab app
          \item Open the Inspector
          \item Turn on Track widget rebuilds
    ...
    \end{column}
  \end{columns}
  \note{Ask a volunteer to run the five steps out loud. Most students expect only the text field to rebuild; the parent Scaffold often rebuilds too, which is the point.}
\end{frame}
