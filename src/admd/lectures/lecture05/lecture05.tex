...
\subtitle{Debugging, Stateless and Stateful widgets, sharing state}
...
\objectivesframe{
  \cell[\faBug]{Debugging}{Launch and attach DevTools, read the inspector, set breakpoints and step through Dart.}
  \cell[\faLayerGroup]{The three trees}{Widget, Element and RenderObject, and what setState actually triggers.}
  \cell[\faToggleOn]{Stateless vs Stateful}{When a widget needs a State object, and when it does not.}
  \cell[\faIcon{share-alt}]{Sharing state}{Lifting state up, InheritedWidget, and why value equality decides rebuilds.}
}

\divider{Part 1 · Debugging}{Debugging Flutter apps}[DevTools, breakpoints, logging]

\begin{frame}[eyebrow=Debugging]{Debugging tools}
  \vskip 2mm
  \begin{grid}[3]
...
\begin{frame}[eyebrow=Debugging]{DevTools tabs}
  \begin{grid}[3]
...
\begin{frame}[eyebrow=Debugging]{The Widget Inspector: selecting and layout}
  ...
  \begin{takeaway}{You cannot reason about layout from the source alone.}
    The Flex explorer shows the constraints your widget received.
  \end{takeaway}
...
\begin{frame}[eyebrow=Debugging]{Rebuild counts and repaint highlighting}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      \lead{Rebuild Stats}{On the Performance view. It counts how many times each widget rebuilt.}
...
\begin{frame}[eyebrow=Debugging]{The frame budget}
  \vskip 4mm
  \begin{grid}[3]
...
\begin{frame}[eyebrow=Debugging]{Frames, jank, and their causes}
  ...
  \begin{itemize}
    ...
    \item The usual causes are covered in this lecture: setState called too high in the tree, expensive work inside build, and a missing const.
  \end{itemize}
  \vskip 3mm
...
\divider{Part 2 · State}{Widget state}[The three trees, setState, Stateless and Stateful widgets]

\begin{frame}[eyebrow=State]{Ephemeral and app state}
  \vskip 3mm
...
\begin{frame}[eyebrow=State]{Widget immutability}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      \begin{itemize}
...
\begin{frame}[eyebrow=State]{setState in detail}
  \begin{columns}[T]
...
\begin{frame}[fragile,eyebrow=State]{Element reuse: runtimeType and key}
\begin{dart}
...
\begin{frame}[eyebrow=State]{Stateless vs Stateful}
  \vskip 2mm
  {\usebeamerfont{small}\begin{tabular}{@{}lp{50mm}p{50mm}@{}}
    ...
    Fits & a label, an icon, a button that only calls a callback & a text field, an animation, a timer \\
    \bottomrule
  \end{tabular}}
  \vskip 4mm
  \begin{takeaway}{A StatelessWidget does not "build once".}
    It rebuilds whenever its parent does. It holds no mutable state of its own.
  \end{takeaway}
...
\begin{frame}[fragile,eyebrow=State]{State object example}
\begin{dart}
...
\begin{frame}[eyebrow=State]{State object rules}
  \vskip 3mm
  \begin{itemize}
...
\divider{Part 3 · Sharing State}{Sharing state}[Lifting state up, InheritedWidget, and value equality]
...
\begin{frame}[eyebrow=Sharing state]{Lifting state up: the cost}
  \vskip 3mm
  \begin{itemize}
    \item The children stay reusable and know nothing about each other.
    ...
    \item That is the problem InheritedWidget solves.
  \end{itemize}
  \vskip 4mm
  \begin{takeaway}{Lifting state up is the default answer.}
    Reach for a package only when the drilling becomes a problem.
  \end{takeaway}
...
\begin{frame}[fragile,eyebrow=Sharing state]{InheritedWidget}
\begin{dart}
...
\begin{frame}[eyebrow=Sharing state]{Value equality and rebuilds}
  \vskip 3mm
  \begin{itemize}
...
\begin{frame}[eyebrow=Sharing state]{Tools for shared state}
  \vskip 1mm
...
\begin{frame}[eyebrow=Sharing state]{Provider, Riverpod, and BLoC}
  \vskip 3mm
  \begin{itemize}
...
\begin{frame}[eyebrow=Recap]{Summary}
  ...
  \begin{enumerate}
    ...
    \item Value equality decides rebuilds.
  \end{enumerate}
