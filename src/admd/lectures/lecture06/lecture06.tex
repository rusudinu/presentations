...
\objectivesframe{
  \cell[\faIcon{exchange-alt}]{Cubit and Bloc}{Write a Cubit and a Bloc, and know which of the two a screen deserves.}
  \cell[\faLayerGroup]{Bloc widgets}{BlocProvider, BlocBuilder, BlocListener, BlocConsumer, read and watch.}
  \cell[\faDatabase]{State and data}{Immutable Equatable states, copyWith, and a repository under the bloc.}
  \cell[\faVial]{Testing and Riverpod}{Test a bloc with bloc\_test, and read Riverpod code when you meet it.}
}

\divider{Part 1 · Why}{Limits of setState}[Shared state, side effects, testing]

\begin{frame}[eyebrow=Why]{Scaling problems with setState}
  \vskip 2mm
  \begin{grid}[3]
...
\begin{frame}[eyebrow=Why]{BLoC data flow}
  \vskip 2mm
  \begin{grid}[3]
...
\begin{frame}[eyebrow=Why]{BLoC terminology}
  \vskip 1mm
...
\divider{Part 2 · Cubit and Bloc}{Cubit and Bloc}[emit, on<Event>, choosing between them]

\begin{frame}[fragile,eyebrow=Cubit]{Defining a Cubit}
  \begin{columns}[T]
...
\end{dart}
  ...
  \end{takeaway}
  \note{This is the smallest useful example, and it is the one in examples/lib/lecture06\_bloc/cubit in the shared repository. Point out that emit is protected: only the Cubit itself may call it, which is what keeps the direction one way.}
\end{frame}

\begin{frame}[fragile,eyebrow=Cubit]{Providing and reading a Cubit}
  \begin{columns}[T]
...
\begin{frame}[fragile,eyebrow=Bloc]{Bloc: event classes}
  \begin{columns}[T]
...
\begin{frame}[fragile,eyebrow=Bloc]{Bloc event handlers}
\begin{dart}
...
\begin{frame}[eyebrow=Bloc]{Cubit versus Bloc}
  \vskip 1mm
  {\usebeamerfont{small}\begin{tabular}{@{}lp{44mm}p{58mm}@{}}
    ...
    Concurrency & none built in          & event transformers, droppable or restartable \\
  ...
  \end{takeaway}
  \note{The ready-made transformers, droppable and restartable, live in the bloc\_concurrency package; the \code{transformer} parameter itself is part of \code{on<Event>}. Discourage the reflex of reaching for Bloc because it looks more professional. The promotion is mechanical: keep the state class, turn each method into an event, and move the body into a handler. That is exactly what Lab 5 Task II asks for.}
\end{frame}

\divider{Part 3 · The widgets}{Bloc widgets}[Provider, builder, listener, consumer]

\begin{frame}[fragile,eyebrow=Widgets]{BlocProvider and scope}
  \begin{columns}[T]
    \begin{column}{0.58\textwidth}
\begin{dart}
MultiBlocProvider(
...
\end{dart}
    ...
    \end{column}
  \end{columns}
  \note{Measure before optimizing: open Rebuild Stats on the DevTools Performance view, which they saw last lecture, and show that a badly placed BlocBuilder repaints the app bar too. buildWhen is a filter, not a fix for a badly shaped state class.}
\end{frame}

\begin{frame}[fragile,eyebrow=Widgets]{BlocListener for side effects}
\begin{dart}
...
\begin{frame}[fragile,eyebrow=Widgets]{BlocConsumer}
\begin{dart}
...
\divider{Part 4 · State and data}{State design and repositories}[Equatable, copyWith, repositories]
...
\begin{frame}[fragile,eyebrow=State]{copyWith and common errors}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
\begin{dart}
TodoState copyWith({
...
\begin{frame}[eyebrow=State]{Choosing a state shape}
  \vskip 2mm
  \begin{grid}[2]
  ...
  \begin{takeaway}{Pick one per feature and stay with it.}
    A screen that keeps data visible while refreshing wants the status field. A screen with disjoint modes wants the sealed hierarchy.
  \end{takeaway}
...
\begin{frame}[eyebrow=Data]{App layers}
  \vskip 2mm
  \begin{grid}[2]
...
\begin{frame}[fragile,eyebrow=Data]{Repository implementation}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
\begin{dart}
...
\begin{frame}[fragile,eyebrow=Data]{Providing repositories at the app root}
  \begin{columns}[T]
    \begin{column}{0.62\textwidth}
\begin{dart}
RepositoryProvider(
...
\end{shell}
    ...
    \begin{column}{0.46\textwidth}
      ...
      \muted{One feature is one folder you could delete in a single command.}
    \end{column}
  \end{columns}
...
\divider{Part 5 · Testing and alternatives}{Testing and Riverpod}[bloc\_test, Riverpod]

\begin{frame}[fragile,eyebrow=Testing]{bloc\_test example}
\begin{dart}
...
\begin{frame}[eyebrow=Testing]{Testing each layer}
  \vskip 2mm
  \begin{grid}[3]
...
\begin{frame}[fragile,eyebrow=Riverpod]{NotifierProvider}
\begin{dart}
...
\end{dart}
  \begin{takeaway}{The same rules apply here.}
    State is replaced, never mutated. \code{ref.watch} in build subscribes, \code{ref.read} in a callback does not. Notifiers are tested like cubits.
  \end{takeaway}
...
\begin{frame}[eyebrow=Riverpod]{BLoC versus Riverpod}
  \vskip 1mm
  ...
  \begin{takeaway}{This course uses flutter\_bloc.}
    Pick the one your team already runs, and never run two state management packages in one app.
  \end{takeaway}
...
\begin{frame}[eyebrow=Mistakes]{Common mistakes}
  \vskip 2mm
  \begin{grid}[2]
