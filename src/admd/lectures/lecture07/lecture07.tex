...
\subtitle{HTTP, dio, retries, JSON code generation, and repositories}
...
\objectivesframe{
  \cell[\faWifi]{HTTP fundamentals}{Methods, safe vs idempotent, and the status codes and headers that change what your app does next.}
  \cell[\faBolt]{Network resilience}{dio's failure paths, interceptors, cancel tokens, and retrying with backoff and jitter.}
  \cell[\faCode]{JSON code generation}{json\_serializable and build\_runner generate the mapping code; freezed generates the model itself.}
  \cell[\faDatabase]{Repository layer}{One class that talks to dio and JSON, so the bloc from Lecture 6 never sees either.}
}

\divider{Part 1 · HTTP}{HTTP fundamentals}[Methods, status codes, headers and timeouts]

\begin{frame}[eyebrow=HTTP]{HTTP request and response}
  \begin{itemize}
  ...
  \begin{takeaway}{One round trip is expensive on mobile.}
    Timeouts, retries and generated JSON all exist because that round trip can be slow or fail.
  \end{takeaway}
...
\begin{frame}[eyebrow=HTTP]{Safe and idempotent methods}
...
\begin{frame}[eyebrow=HTTP]{Common status codes}
...
\begin{frame}[eyebrow=Numbers]{Timeout budgets}
  \vskip 4mm
  \begin{grid}[3]
...
\divider{Part 2 · dio and http}{HTTP clients}[Requests, failure paths, interceptors, package:http]

\begin{frame}[fragile,eyebrow=dio]{dio requests and failure paths}
  \begin{columns}[T]
...
\begin{frame}[fragile,eyebrow=dio]{dio interceptors}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
\begin{dart}
...
\begin{frame}[fragile,eyebrow=dio]{Cancel tokens}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
\begin{dart}
CancelToken? _token;
...
\end{dart}
    ...
    \begin{column}{0.40\textwidth}
      ...
      \begin{itemize}
        ...
        \item A canceled request throws \code{DioExceptionType.cancel}, which you ignore.
...
\begin{frame}[fragile,eyebrow=HTTP]{package:http}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
\begin{dart}
...
\divider{Part 3 · Resilience}{Handling network failures}[Retry with backoff, jitter, retry policy]
...
\begin{frame}[eyebrow=Resilience]{Retryable failures}
  \vskip 1mm
  ...
  \end{tabular}}
  \vskip 1mm
  \begin{takeaway}{Retry only calls where a repeat is harmless.}
    Safe methods, plus POST with an idempotency key.
  \end{takeaway}
...
\begin{frame}[eyebrow=Resilience]{Thundering herds and hedged requests}
  \begin{itemize}
    ...
    \item \textbf{Hedging is the opposite problem.} A few requests are slow. Send a second copy after a short delay, take whichever answers first, cancel the other.
  \end{itemize}
...
\divider{Part 4 · JSON}{JSON serialization}[fromJson, toJson, code generation, freezed]

\begin{frame}[fragile,eyebrow=JSON]{Manual JSON parsing in Dart}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
\begin{dart}
class Reading {
...
\end{dart}
    ...
    \begin{column}{0.40\textwidth}
      ...
      \begin{itemize}
        \item A renamed key fails at runtime, never at compile time.
        \item Every model needs this twice, kept in sync by hand.
        \item Nested objects multiply the boilerplate fast.
      \end{itemize}
    \end{column}
  \end{columns}
  \note{Ask the room to spot the bug if device\_id were renamed on the server without updating this file: nothing turns red until the app runs, the cast throws there, and a nullable field would instead carry a null three screens further. The key name is a string literal, so no tool checks it.}
\end{frame}

\begin{frame}[fragile,eyebrow=JSON]{@JsonSerializable}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
\begin{dart}
part 'reading.g.dart';
...
class Reading {
  ...
  @JsonKey(name: 'temp_c')
  final double tempC;
  factory Reading.fromJson(Json j) =>
      _$ReadingFromJson(j);
  Json toJson() => _$ReadingToJson(this);
}
\end{dart}
    \end{column}
    \begin{column}{0.40\textwidth}
...
\end{shell}
      \begin{itemize}
        ...
        \item \code{Json} is a typedef for \code{Map<String, dynamic>}, the type the generated code expects; declare it once.
        \item Commit \code{.g.dart} files, or generate them in CI.
              \end{itemize}
    \end{column}
  \end{columns}
  \note{Two files, one model: the developer writes the class and its annotations, build\_runner writes the mapping code into reading.g.dart. A missing key then throws at the boundary instead of turning into a silent null three screens later. Run the watch command during development so it regenerates on every save.}
\end{frame}
...
class Task {
  ...
  factory Task.fromJson(Json j) => _$TaskFromJson(j);
}
\end{dart}
    \end{column}
    \begin{column}{0.40\textwidth}
...
\begin{frame}[eyebrow=JSON]{Code generation packages}
  \vskip 1mm
  ...
  \end{tabular}}
  \vskip 1mm
  \muted{build\_runner is slow on large projects. It now deletes stale outputs itself, with no extra flag.}
...
\begin{frame}[fragile,eyebrow=JSON]{freezed data classes}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
\begin{dart}
...
\end{dart}
    ...
    \end{column}
  \end{columns}
  \note{The \code{sealed} keyword is required: current freezed versions ask for \code{sealed} on a union and \code{abstract} on a single-constructor class, and the exhaustive switch depends on it. Because the three factories are the only ways to build a TaskState, Dart catches a missing case at compile time, not in production. Compare this class to the hand-written state class from Lecture 6.}
\end{frame}

\begin{frame}[eyebrow=JSON]{Choosing a JSON approach}
  \vskip 1mm
...
\divider{Part 5 · Architecture}{Network layer architecture}[Repositories, API keys]

\begin{frame}[fragile,eyebrow=Architecture]{API repository class}
\begin{dart}
...
\begin{frame}[eyebrow=Architecture]{Benefits of a repository layer}
  \vskip 2mm
...
\begin{frame}[eyebrow=Architecture]{API keys and secrets}
  \begin{itemize}
...
\begin{frame}[eyebrow=Architecture]{Example: adding a task}
  \begin{enumerate}
...
\begin{frame}[eyebrow=Recap]{Summary}
  ...
  \begin{enumerate}
    ...
    \item Generate the JSON mapping, do not hand-write it. \muted{json\_serializable keeps the mapping in sync with the class; a hand-written one drifts until it fails at runtime.}
  \end{enumerate}
  \vskip 5mm
