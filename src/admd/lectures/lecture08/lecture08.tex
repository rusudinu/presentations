...
\objectivesframe{
  \cell[\faCloud]{Firebase setup}{flutterfire configure, a Firestore read and write behind a security rule.}
  \cell[\faIcon{box}]{Package dependencies}{Read a pub.dev page, pin a version constraint, and know what pubspec.lock is for.}
  \cell[\faToggleOn]{Feature flags}{Firebase Remote Config: defaults, fetch and activate, and a kill switch.}
  \cell[\faIcon{exchange-alt}]{REST and GraphQL}{Over- and under-fetching, a GraphQL schema, and when each style wins.}
}

\divider{Part 1 · Firebase}{Firebase and Firestore}[flutterfire, Firestore, security rules, Cloud Functions]

\begin{frame}[eyebrow=Firebase]{Firebase overview}
  \vskip 2mm
  \begin{grid}[3]
...
\begin{frame}[eyebrow=Firebase]{Firestore data model}
  \vskip 2mm
  \begin{enumerate}
...
\begin{frame}[fragile,eyebrow=Firebase]{Firebase setup in Flutter}
\begin{shell}
# once per machine
...
\begin{frame}[fragile,eyebrow=Firebase]{Firestore security rules}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
\begin{codeblock}
...
\end{codeblock}
    ...
    \begin{column}{0.46\textwidth}
      ...
      \muted{Never store a password or an API key in a document. Anything that needs a secret belongs behind a Cloud Function.}
    \end{column}
  \end{columns}
...
\begin{frame}[eyebrow=Firebase]{Cloud Functions and Firebase limits}
  \vskip 1mm
  \begin{enumerate}
    ...
    \item \textbf{Use them for what the client must not do:} verifying a receipt, calling a paid API with your secret key, sending a push to someone else's device.
  \end{enumerate}
...
\end{dart}
  ...
  \begin{takeaway}{The client sends a token, never an identity.}
    The function is the only place that decides who the caller is.
  \end{takeaway}
...
\divider{Part 2 · Packages}{Package dependencies}[pub.dev, version constraints, pubspec.lock]

\begin{frame}[eyebrow=Packages]{Choosing a package}
  \vskip 2mm
  \begin{grid}[3]
...
\begin{frame}[fragile,eyebrow=Packages]{Version constraints and caret syntax}
  \vskip 1mm
...
\begin{frame}[eyebrow=Packages]{pubspec.lock and version control}
  \vskip 2mm
  \begin{enumerate}
    ...
    \item flutter pub get obeys the lockfile. flutter pub upgrade rewrites it.
  \end{enumerate}
...
\divider{Part 3 · Feature flags}{Feature flags}[Firebase Remote Config, staged rollout, kill switch]

\begin{frame}[eyebrow=Feature flags]{Deployment versus release}
  \begin{columns}[T]
    ...
    \end{column}
  \end{columns}
  \begin{takeaway}{The kill switch is the main benefit.}\end{takeaway}
...
\begin{frame}[eyebrow=Feature flags]{Firebase Remote Config}
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      \begin{itemize}
...
\begin{frame}[eyebrow=Feature flags]{Feature flag guidelines}
  \vskip 2mm
  \begin{enumerate}
...
\divider{Part 4 · APIs}{REST and GraphQL}[REST, GraphQL, round trips]

\begin{frame}[eyebrow=APIs]{REST verbs and semantics}
  \vskip 1mm
...
\begin{frame}[fragile,eyebrow=APIs]{GraphQL mutations and schema benefits}
  \begin{columns}[T]
    \begin{column}{0.42\textwidth}
\begin{codeblock}
...
\begin{frame}[eyebrow=APIs]{REST vs GraphQL comparison}
  {\renewcommand{\arraystretch}{1.0}\usebeamerfont{small}\begin{tabular}{@{}lp{44mm}p{44mm}@{}}
    ...
    \bottomrule
  \end{tabular}}
  \begin{takeaway}{Simple resources favor REST. Related data and several clients favor GraphQL.}\end{takeaway}
