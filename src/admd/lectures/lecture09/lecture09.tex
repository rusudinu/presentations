...
\subtitle{Analytics, crash reporting, alerts, local storage, and app lifecycle}
...
\objectivesframe{
  \cell[\faChartLine]{Production monitoring}{Log the events, traces and crash reports that describe a device you will never touch.}
  \cell[\faBell]{Metrics and alerting}{Crash-free users per version, p95 instead of the average, and alerts that have an owner.}
  \cell[\faDatabase]{Local storage}{Preferences, secure storage, a SQL database, files, and a cache with a time to live.}
  \cell[\faLock]{Secure storage}{What belongs in the keychain, what belongs on the server, and what to clear on sign-out.}
}

\divider{Part 1 · Observability}{Observability}[Events, metrics, crashes, alerts]

\begin{frame}[eyebrow=Observability]{Mobile observability challenges}
  \vskip 2mm
  \begin{itemize}
...
\begin{frame}[eyebrow=Observability]{Types of observability signals}
  \vskip 2mm
  \begin{grid}[2]
  ...
  \end{grid}
  \begin{takeaway}{Each layer answers a different question.}\end{takeaway}
...
\begin{frame}[eyebrow=Observability]{Event naming and personal data}
  \vskip 2mm
  \begin{itemize}
    ...
    \item Never put personal data in a parameter: no emails, no names, no free text a user typed. That is a legal question about personal data.
...
\end{dart}
  \muted{The first handler catches framework errors. The second catches async errors outside it. Reports upload on the next launch.}
...
\begin{frame}[fragile,eyebrow=Observability]{Reporting caught errors}
  \begin{columns}[T]
...
\end{dart}
    ...
    \end{column}
  \end{columns}
  \begin{takeaway}{Report the errors you catch.}\end{takeaway}
...
\begin{frame}[eyebrow=Observability]{Crash-free users}
  \vskip 3mm
  \begin{grid}[3]
...
\begin{frame}[eyebrow=Observability]{Latency percentiles}
  \vskip 1mm
  ...
  \begin{takeaway}{A percentile describes a defined share of your users.}\end{takeaway}
...
\end{dart}
    ...
    \begin{column}{0.46\textwidth}
      \lead{Some traces cost you nothing.}{App start, app in foreground and app in background are captured as soon as the package is added. Per-screen rendering is not: Flutter draws into one native view.}
      \vskip 2mm
      \muted{Dart HTTP calls are not traced either. \code{dart:io} bypasses the instrumented native stack, so time the call yourself.}
    \end{column}
  \end{columns}
  \begin{takeaway}{Give every trace a budget before you read it.}\end{takeaway}
...
\begin{frame}[eyebrow=Observability]{Alerting guidelines}
  \vskip 1mm
  {\usebeamerfont{small}\begin{tabular}{@{}lp{46mm}p{54mm}@{}}
    ...
    \bottomrule
  \end{tabular}}
  \begin{takeaway}{Every alert needs an owner and an action.}\end{takeaway}
...
\begin{frame}[eyebrow=Observability]{Incident walkthrough}
  \vskip 1mm
...
\divider{Part 2 · Local storage}{Local storage}[Preferences, secrets, a database, files, a cache]

\begin{frame}[eyebrow=Storage]{Storage options}
  \vskip 1mm
  {\usebeamerfont{small}\begin{tabular}{@{}lp{44mm}p{52mm}@{}}
    ...
    \bottomrule
  \end{tabular}}
  \begin{takeaway}{Size and shape pick the store.}
...
\end{dart}
    ...
    \begin{column}{0.42\textwidth}
      ...
      \vskip 2mm
      \muted{Reads are synchronous once the file is loaded, so keep it small. Nothing here is encrypted. This API is legacy: new code should use SharedPreferencesAsync.}
    \end{column}
  \end{columns}
...
\end{dart}
    \end{column}
    \begin{column}{0.42\textwidth}
      \lead{The platform holds the key.}{iOS and macOS use the Keychain. On Android the key lives in the Keystore and the values are stored as AES-GCM ciphertext.}
      \vskip 2mm
...
\begin{frame}[eyebrow=Storage]{Sensitive data}
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      \vskip 1mm
      \begin{itemize}
    ...
    \end{column}
  \end{columns}
  \begin{takeaway}{Put in encrypted storage only the few values you cannot avoid keeping on the device.}\end{takeaway}
...
\begin{frame}[fragile,eyebrow=Storage]{Drift table definition}
\begin{dart}
...
\begin{frame}[fragile,eyebrow=Storage]{Watched queries in Drift}
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
\begin{dart}
...
\begin{frame}[eyebrow=Storage]{App directories and cleanup}
  \vskip 1mm
  {\usebeamerfont{small}\begin{tabular}{@{}lp{22mm}p{54mm}@{}}
    ...
    \bottomrule
  \end{tabular}}
  \begin{takeaway}{Anything in the temporary directory can vanish between launches.}\end{takeaway}
...
\begin{frame}[fragile,eyebrow=Storage]{In-memory cache with TTL}
\begin{dart}
class Cached<T> {
...
\begin{frame}[eyebrow=Storage]{Repository read strategy}
  \vskip 2mm
  \begin{enumerate}
...
\divider{Part 3 · Lifecycle}{App lifecycle}

\begin{frame}[eyebrow=Lifecycle]{Lifecycle states}
  \vskip 2mm
  \begin{grid}[2]
  ...
  \end{takeaway}
  \note{There is a fifth state, hidden, which fires just before paused on every platform; treat the two together. On Android the process can be killed after paused without another callback, and in detached you cannot count on the network or the UI. Ask what happens to a half-typed message when the user takes a call and the system reclaims memory. Nothing in the framework saves it for you.}
\end{frame}
...
\begin{frame}[eyebrow=Lifecycle]{Storage checklist}
  \vskip 2mm
  \begin{checklist}
...
\begin{frame}[eyebrow=Next]{Tasks for this week}
  \begin{columns}[T]
    ...
    \end{column}
  \end{columns}
  \begin{takeaway}{The lab app carries the third item forward.}
    Open the pull request even if the database half is unfinished, so the review catches the schema early.
  \end{takeaway}
  \note{The cumulative lab app builds on the first and third items. Tell students to open a pull request even if the database half is incomplete, so the review can catch the schema early.}
\end{frame}
