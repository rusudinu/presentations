...
\divider{Part 1 · Fundamentals}{Authentication fundamentals}

\begin{frame}[eyebrow=Fundamentals]{Authentication and authorization}
  \vskip 2mm
  \begin{columns}[T]
...
\begin{frame}[eyebrow=Fundamentals]{Authentication factors and MFA}
  ...
  \begin{takeaway}{Multi-factor authentication (MFA) needs factors from two different rows.}
    A password plus a security question is one factor, asked twice. Firebase supports SMS and a time-based one-time password (TOTP) as a second factor.
  \end{takeaway}
...
\begin{frame}[eyebrow=Fundamentals]{Managed authentication}
  \vskip 2mm
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
...
\divider{Part 2 · firebase\_auth}{Firebase Auth in Dart}[Sign-up, sign-in, account lifecycle]

\begin{frame}[eyebrow=Firebase Auth]{Auth providers and packages}
...
\begin{frame}[fragile,eyebrow=Firebase Auth]{Signing in and error messages}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
\begin{dart}
...
\begin{frame}[eyebrow=Firebase Auth]{Email enumeration protection}
  \vskip 2mm
...
\begin{frame}[fragile,eyebrow=Firebase Auth]{authStateChanges}
\begin{dart}
...
\begin{frame}[eyebrow=Firebase Auth]{Auth state streams}
  \vskip 2mm
  \begin{enumerate}
...
\end{dart}
  \begin{takeaway}{Enforce email verification.}
...
\divider{Part 3 · OAuth and tokens}{OAuth and identity tokens}[Credential handoff, ID tokens]

\begin{frame}[eyebrow=OAuth]{Client-side credential handoff}
  \vskip 1mm
  \begin{enumerate}
    \item \textbf{The app asks Google to sign the user in.} \muted{\code{GoogleSignIn.instance.authenticate()}}
  ...
  \begin{takeaway}{Every step above runs on the device.}
    Firebase never sees the Google password: it checks Google's signature, then mints its own session.
  \end{takeaway}
...
\begin{frame}[fragile,eyebrow=OAuth]{Google sign-in flow}
\begin{dart}
...
\begin{frame}[eyebrow=OAuth]{Social sign-in setup}
  \vskip 3mm
  \begin{itemize}
...
\begin{frame}[fragile,eyebrow=Tokens]{Backend token verification}
\begin{dart}
...
\divider{Part 4 · Data, rules, and App Check}{Data protection and App Check}[User profiles, security rules, App Check]

\begin{frame}[eyebrow=User data]{Password storage}
  \vskip 3mm
  The identity provider holds the credential. Firebase keeps a salted hash on Google's infrastructure; once account creation returns, the plaintext password is gone from your process.
  \vskip 3mm
...
\begin{frame}[fragile,eyebrow=User data]{User profile documents}
\begin{dart}
...
\end{dart}
  \begin{takeaway}{The uid is the join key.}
    It connects the identity system to your data, and that join is what the next two slides defend.
  \end{takeaway}
...
\begin{frame}[fragile,eyebrow=Authorization]{Uid-based security rules}
...
\begin{frame}[eyebrow=Authorization]{Insecure rule example}
  \vskip 1mm
  ...
  \end{itemize}
  \begin{takeaway}{Rules run on Google's servers, so a client cannot bypass them.}\end{takeaway}
...
\begin{frame}[eyebrow=App Check]{App Check overview}
  \vskip 3mm
  Your Firebase configuration ships inside the binary: an API key, a project ID, an app ID. It identifies the project. It was never a secret.
  \vskip 3mm
...
\begin{frame}[eyebrow=App Check]{App Check, Auth, and security rules}
  \vskip 3mm
  \begin{grid}[3]
...
\begin{frame}[fragile,eyebrow=App Check]{Enabling App Check}
\begin{dart}
...
\begin{frame}[eyebrow=App Check]{Choosing an attestation provider}
  \vskip 2mm
...
\begin{frame}[eyebrow=App Check]{Enforcement and debug tokens}
  \vskip 1mm
  \begin{enumerate}
  ...
  \end{takeaway}
  \note{Enforcing before old app versions have drained is the classic outage: every user still on a build without activate() is locked out at once. The metrics page exists precisely so you can time this safely. One product does not wait for you: from November 2026 App Check is enforced automatically for Firebase AI Logic, the path Lecture 12 uses.}
\end{frame}
