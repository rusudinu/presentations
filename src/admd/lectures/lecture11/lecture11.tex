...
\subtitle{WebSockets, runtime permissions, and routing with go\_router}
...
\objectivesframe{
  ...
  \cell[\faIcon{sync-alt}]{Reliable connections}{Backoff, heartbeats, wss, and what the OS does to a socket when the app leaves the screen.}
  \cell[\faLock]{Runtime permissions}{Manifest and plist entries, Android 13+ granular media, and every state permission\_handler returns.}
  \cell[\faRoute]{Routing with go\_router}{Routes as data, parameters, nested shells, and a redirect guard wired to auth state.}
}

\divider{Part 1 · Realtime}{Realtime connections}[WebSockets, SSE, long polling, MQTT]

\begin{frame}[eyebrow=Realtime]{HTTP polling versus WebSockets}
  ...
  {\usebeamerfont{small}\begin{tabular}{@{}lp{44mm}p{44mm}@{}}
    ...
    \bottomrule
  \end{tabular}}
  \note{Ask the room how they would build a live chat with plain HTTP. Most say polling; walk through what that costs at scale.}
\end{frame}
...
\begin{frame}[eyebrow=Realtime]{Push transports compared}
  {\usebeamerfont{small}\begin{tabular}{@{}llp{28mm}p{54mm}@{}}
    ...
    \bottomrule
  \end{tabular}}
  \begin{takeaway}{In MQTT, a broker fans one reading out to every subscriber.}
    MEC Lecture 4 covers the broker side.
  \end{takeaway}
...
\begin{frame}[fragile,eyebrow=Realtime]{web\_socket\_channel: connection lifecycle}
  \begin{columns}[T]
...
\begin{frame}[eyebrow=Realtime]{Transport encryption with wss://}
...
\begin{frame}[eyebrow=Realtime]{WebSocket authentication}
  ...
  \begin{takeaway}{Pick the one your server and proxies support, and keep the token short-lived.}
  \end{takeaway}
...
\begin{frame}[fragile,eyebrow=Realtime]{Heartbeats and dead connections}
  \begin{columns}[T]
...
\begin{frame}[fragile,eyebrow=Realtime]{Reconnection with backoff and jitter}
  \begin{columns}[T]
    \begin{column}{0.60\textwidth}
\begin{dart}
...
\begin{frame}[eyebrow=Realtime]{Sockets in the background}
  \begin{columns}[T]
    \begin{column}{0.58\textwidth}
  ...
  \begin{takeaway}{A common realtime bug on mobile.}
    It works foreground and breaks the moment the phone locks. Give every message a server-side id so resync is a gap query.
  \end{takeaway}
...
\divider{Part 2 · Permissions}{Runtime permissions}[Manifest, Info.plist, permission\_handler statuses]

\begin{frame}[eyebrow=Permissions]{Declaring and requesting permissions}
  \begin{columns}[T]
    \begin{column}{0.58\textwidth}
...
\begin{frame}[eyebrow=Permissions]{Android 13+ granular media permissions}
...
\begin{frame}[eyebrow=Permissions]{Permission statuses}
...
\begin{frame}[eyebrow=Permissions]{Limited and provisional access}
  \vskip 2mm
...
\begin{frame}[fragile,eyebrow=Permissions]{permission\_handler and permanent denial}
  \begin{columns}[T]
    \begin{column}{0.60\textwidth}
\begin{dart}
...
\begin{frame}[eyebrow=Permissions]{Permission request guidelines}
  \begin{grid}[3]
...
\divider{Part 3 · Routing}{Declarative routing}[go\_router, route parameters, ShellRoute, redirect guards]

\begin{frame}[fragile,eyebrow=Routing]{Navigator 1.0 and its limits}
  \begin{columns}[T]
...
\begin{frame}[fragile,eyebrow=Routing]{go\_router route configuration}
  \begin{columns}[T]
    \begin{column}{0.60\textwidth}
\begin{dart}
...
\end{dart}
    \end{column}
    \begin{column}{0.36\textwidth}
      \lead{Stack equals a function of state}{You declare what exists, instead of saying "go here".}
      \muted{One \code{MaterialApp.router} line wires it in, then it works from a URL bar.}
...
\begin{frame}[eyebrow=Routing]{Passing data to a route}
...
\begin{frame}[eyebrow=Routing]{ShellRoute and persistent navigation}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      A bottom navigation bar is a frame around screens. ShellRoute gives a nested Navigator: the Scaffold and its bar are built once, and only the body swaps.
...
\begin{frame}[fragile,eyebrow=Routing]{redirect: an app-wide auth guard}
  \begin{columns}[T]
    \begin{column}{0.60\textwidth}
\begin{dart}
final router = GoRouter(
...
\end{dart}
    \end{column}
    \begin{column}{0.36\textwidth}
      \lead{The bridge back to Lecture 10}{redirect runs before every navigation, first one included. One place decides who sees what.}
      \muted{authRefresh wraps authStateChanges() in a ChangeNotifier, so sign-out re-runs the guard.}
...
\begin{frame}[eyebrow=Synthesis]{Worked example: nearby chat}
  \vskip 2mm
  \begin{grid}[3]
...
\begin{frame}[eyebrow=Recap]{Summary}
  \vskip 2mm
  \begin{enumerate}
    \item A WebSocket is one held-open socket. \muted{Keep it alive with wss, heartbeats and jittered backoff, and close it in dispose().}
  ...
  \end{enumerate}
  \vskip 5mm
  \kv{Further reading}{\link{https://pub.dev/packages/web_socket_channel}{web\_socket\_channel} \textperiodcentered\ \link{https://pub.dev/packages/permission_handler}{permission\_handler} \textperiodcentered\ \link{https://pub.dev/packages/go_router}{go\_router} \textperiodcentered\ RFC 6455}
