...
\objectivesframe{
  \cell[\faBalanceScale]{On-device versus cloud inference}{The variables that decide it: latency, privacy, offline behavior, cost per call, capability ceiling.}
  \cell[\faMicrochip]{ML Kit and LiteRT}{Real Dart for text and barcodes, plus your own quantized model through tflite\_flutter.}
  \cell[\faKey]{API key placement}{Why a key inside an app binary is public, and the two architectures that fix it.}
  \cell[\faIcon{wave-square}]{LLM integration in the UI}{Streaming answers from a large language model, timeouts, token cost, and validating output you cannot trust.}
}

\divider{Part 1 · The decision}{On-device versus cloud inference}[Latency, privacy, offline use, cost, capability ceiling]

\begin{frame}[eyebrow=Decision]{Kinds of AI in a mobile app}
  \vskip 2mm
  \begin{grid}[3]
  ...
  \end{grid}
  \vskip 2mm
  \muted{The three shapes of artificial intelligence (AI) in an app differ in where the compute happens, what happens with no network, and what an attacker can reach.}
  \begin{takeaway}{Most shipping apps use two of the three.}
    Picking one is an architecture decision, and it is expensive to reverse late.
  \end{takeaway}
...
\begin{frame}[eyebrow=Decision]{On-device versus cloud comparison}
  \vskip 1mm
...
\begin{frame}[eyebrow=Decision]{Model size and device memory}
  \vskip 2mm
  \begin{grid}[3]
...
\begin{frame}[eyebrow=Decision]{Hybrid architecture}
  \vskip 2mm
  \begin{columns}[T]
...
\begin{frame}[eyebrow=Decision]{Escalation rate}
  \vskip 2mm
  \begin{grid}[2]
...
\divider{Part 2 · On the device}{ML Kit, LiteRT, and quantization}

\begin{frame}[eyebrow=On-device ML]{Google ML Kit APIs}
  \vskip 2mm
  \begin{grid}[3]
...
\begin{frame}[eyebrow=On-device ML]{More ML Kit APIs and setup}
  \vskip 2mm
  \begin{grid}[3]
  ...
  \begin{takeaway}{These are community-maintained Dart wrappers.}
    Each call crosses a platform channel into the native ML Kit SDK. Fine for a photo; it is why the next slide is about back-pressure.
  \end{takeaway}
...
\begin{frame}[fragile,eyebrow=On-device ML]{Text recognition from an image}
  \begin{columns}[T]
...
\begin{frame}[eyebrow=On-device ML]{On-device generative AI availability}
  \begin{columns}[T]
...
\begin{frame}[eyebrow=Custom models]{Custom models with LiteRT}
  \begin{columns}[T]
...
\begin{frame}[fragile,eyebrow=Custom models]{tflite\_flutter: loading, inference, isolates}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
\begin{dart}
...
\begin{frame}[eyebrow=Custom models]{Quantization}
  \vskip 3mm
...
\divider{Part 3 · Cloud LLMs}{Cloud LLM APIs}[API key placement, backend proxy, Firebase AI Logic]

\begin{frame}[fragile,eyebrow=Cloud LLMs]{API key embedded in the app}
  \begin{columns}[T]
...
\begin{frame}[fragile,eyebrow=Cloud LLMs]{Extracting a key from an app binary}
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
\begin{shell}
...
\end{shell}
    ...
    \begin{column}{0.43\textwidth}
      ...
      \vskip 1mm
      \muted{A leaked key is billed to the account that issued it until you notice.}
    \end{column}
  \end{columns}
...
\begin{frame}[eyebrow=Cloud LLMs]{Option A: backend proxy}
  \vskip 3mm
  \begin{enumerate}
...
\begin{frame}[eyebrow=Cloud LLMs]{Option B: Firebase AI Logic and App Check}
  \vskip 3mm
  \begin{enumerate}
...
\begin{frame}[fragile,eyebrow=Cloud LLMs]{firebase\_ai with App Check}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
\begin{dart}
...
\begin{frame}[fragile,eyebrow=Cloud LLMs]{Streaming responses}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
\begin{dart}
...
\end{dart}
    ...
    \begin{column}{0.45\textwidth}
      ...
      \vskip 1mm
      \lead{Cancel the stream.}{On dispose, or when a new question starts.}
    \end{column}
  \end{columns}
...
\divider{Part 4 · Engineering reality}{Cost, failures, and incorrect output}

\begin{frame}[eyebrow=Engineering reality]{Tokens and billing}
  \vskip 3mm
  \begin{grid}[3]
...
\begin{frame}[eyebrow=Engineering reality]{Common failure modes}
  \vskip 1mm
  ...
  \end{tabular}}
  \vskip 1mm
  \muted{Set a budget for the whole feature, not one attempt: three tries of ten seconds each is a 30-second wait.}
...
\begin{frame}[fragile,eyebrow=Engineering reality]{Output validation}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
...
\begin{frame}[eyebrow=Recap]{Summary}
  \vskip 2mm
  \begin{enumerate}
    \item On-device or cloud is an architecture decision. \muted{Latency, privacy, offline behavior, cost and capability ceiling.}
