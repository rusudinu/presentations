...
\begin{frame}[eyebrow=Custom models]{Quantization: shrinking the model to fit}
  ...
  \end{takeaway}
  \note{Firebase machine learning custom model hosting can own the download-and-version problem for you, with rollout and rollback, if the class asks about it.}
\end{frame}
...
\begin{frame}[eyebrow=Cloud LLMs]{Shape B: Firebase AI Logic and App Check}
  ...
  \end{enumerate}
  \vskip 5mm
  \muted{Working the same afternoon, with attestation built in, but Google models only, and only once you press Enforce in the App Check console.}
  ...
  \end{takeaway}
  \note{Ask why Enforce is a separate switch from Activate. Activation only measures traffic; enforcement is the step that turns away requests without a valid App Check token.}
\end{frame}
...
\begin{frame}[fragile,eyebrow=Cloud LLMs]{firebase\_ai: the call with no key in it}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
\begin{dart}
// Firebase.initializeApp already ran (Lecture 8)
...
\end{dart}
    ...
    \begin{column}{0.38\textwidth}
      ...
      \vskip 2mm
      \lead{activate() only measures.}{Enforcement is a separate switch you flip in the console, per product.}
    \end{column}
  \end{columns}
...
\begin{frame}[eyebrow=Engineering reality]{Tokens: the unit you are billed in}
  ...
  \end{grid}
  \vskip 2mm
  \muted{Cap the output with maxOutputTokens, and cap the input by summarizing history instead of resending all of it. Route by difficulty: a small model answers most questions, only its failures reach the expensive one.}
...
\begin{frame}[eyebrow=Recap]{Further reading}
  ...
  \vskip 3mm
  \kv{LiteRT}{\link{https://ai.google.dev/edge/litert}{ai.google.dev/edge/litert}}
  \vskip 3mm
