...
\begin{frame}[eyebrow=Model]{The context window is the only memory}
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      ...
      \vskip 1mm
      \muted{Nothing outside it exists. A model has no memory between calls, so every turn resends the history.}
      \vskip 1mm
      \muted{When the history stops fitting, something is dropped, and the model behaves as if it never happened.}
    \end{column}
    \begin{column}{0.42\textwidth}
...
\end{dart}
  ...
  \begin{warning}[Nothing in the model checks the world]
    A wrong answer comes out of exactly the machinery that produces a right one, so it arrives in the same confident tone. Temperature zero removes the randomness, not the error.
  \end{warning}
...
\begin{frame}[eyebrow=Model]{Training, fine-tuning, and prompting}
  ...
  {\usebeamerfont{small}\renewcommand{\arraystretch}{1.05}\begin{tabular}{@{}lp{3.4cm}p{3.6cm}p{2.6cm}@{}}
    ...
    \midrule
    Pre-training & every weight, from scratch & thousands of accelerator-days & never \\
    Fine-tuning & the weights, slightly & a labeled dataset and a training run & rarely, for a fixed style or format \\
    Prompting and retrieval & only the input of one call & one request & always \\
    \bottomrule
  \end{tabular}}
  \vskip 2mm
...
\begin{frame}[eyebrow=Model]{Where the model runs, and what it costs}
  ...
  \begin{grid}[3]
    ...
    \cell[\faServer]{Open weights, local}{LM Studio or Ollama serve a downloaded model, such as one from the Qwen family, on your own machine behind an OpenAI-compatible endpoint.}
  ...
  \end{grid}
  \vskip 3mm
  \muted{Two numbers decide how a feature feels: input plus output tokens is the bill, and time to first token is what the user experiences. Streaming turns one long wait into a short one.}
  \begin{takeaway}{The request shape is the same in all three.}
    A local endpoint is the cheapest way to develop against the loops in this lecture.
  \end{takeaway}
...
\begin{frame}[eyebrow=Prompting]{The four parts of a prompt}
  \vskip 2mm
  \begin{grid}[2]
    \cell[\faIcon{cog}]{System prompt}{Role, rules, refusals, output format. Sent on every turn, invisible to the user, and the only place your policy lives.}
    \cell[\faIcon{list-ol}]{Few-shot examples}{Two or three input and output pairs teach a format faster than a paragraph of adjectives ever will.}
    \cell[\faIcon{comment}]{The user turn}{The question, plus whatever your app attaches to it: the selected item, retrieved passages, tool results.}
    \cell[\faIcon{sliders-h}]{Generation config}{Temperature, the output cap, the response schema. Steering that lives in code, not in prose.}
  \end{grid}
  \vskip 3mm
  \begin{takeaway}{A prompt is source code you cannot compile.}
    Version it, review it in a pull request, and keep a fixed set of questions that must still pass after you edit it.
  \end{takeaway}
...
final model = FirebaseAI.googleAI().generativeModel(
  ...
  systemInstruction: Content.system(rules),
  generationConfig: GenerationConfig(
    temperature: 0.1,
...
\end{dart}
  \muted{JavaScript Object Notation (JSON) with a schema replaces prose parsing. The schema constrains the shape of the answer, never its truth: parse it, then check every value against your own data before it reaches the user interface (UI).}
...
\begin{frame}[eyebrow=Prompting]{What actually limits an injection}
  \vskip 2mm
  \begin{grid}[2]
    \cell[\faLock]{Least privilege}{The model reaches only the tools one signed-in user may use, scoped to that user's own data, and nothing else.}
    \cell[\faUserCheck]{Confirm side effects}{A tool that spends, deletes, or messages another person stops and shows the user exactly what is about to happen.}
    \cell[\faFilter]{Validate on the way out}{Look every identifier up in your database. Check links against an allowlist. Reject values outside the schema.}
    \cell[\faIcon{shield-alt}]{Keep the channels apart}{Retrieved text goes in as labeled data. It is never concatenated into the rules section of the system prompt.}
  \end{grid}
  \vskip 3mm
  \begin{takeaway}{Injection is contained by architecture, not by wording.}
    Assume the prompt is already compromised, then ask what the model is still able to do.
  \end{takeaway}
...
\begin{frame}[eyebrow=Retrieval]{Chunking decides retrieval quality}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      ...
      \vskip 1mm
      \muted{A workable default: a few hundred tokens per chunk, split on headings and paragraphs, with a small overlap so a sentence on a boundary survives.}
    \end{column}
    \begin{column}{0.44\textwidth}
      \begin{hint}[Store metadata next to the vector]
        The source file, the section title, the owner's user id and a timestamp. The title is what you cite, and the owner id is how a shared index stays private.
      \end{hint}
    \end{column}
  \end{columns}
...
\begin{frame}[eyebrow=Retrieval]{Where the vectors live}
  ...
  {\usebeamerfont{small}\renewcommand{\arraystretch}{1.05}\begin{tabular}{@{}lp{3.5cm}p{5.4cm}@{}}
    ...
    \midrule
    Vertex AI Vector Search & Google Cloud, managed & a large shared corpus outgrows one machine \\
    pgvector & inside your PostgreSQL & rows and vectors belong together, with filters and joins \\
    Elasticsearch & your own cluster & you already run it, and want hybrid search \\
    Chroma or FAISS & a small backend process & prototypes and corpora that fit in memory \\
    A table on the phone & the device & a private, user-sized corpus that must work offline \\
    \bottomrule
  \end{tabular}}
  \vskip 2mm
  \begin{takeaway}{A few thousand chunks need no vector index.}
    A plain table and a cosine loop are fast enough. An index earns its complexity when a linear scan stops fitting the time budget.
  \end{takeaway}
...
\end{dart}
  \muted{The system prompt carries the grounding rule: answer only from the context, say you do not know when it is missing, and cite the source labels. The label is what turns an answer into a citation the user can open.}
  \muted{The owner filter is not a nicety. A shared index queried without it is a data leak dressed up as a feature.}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.42\textwidth}
      \begin{hint}[Keep the set small]
        Every declaration costs input tokens on every single turn and gives the model one more thing to confuse. Five sharp tools beat twenty vague ones.
      \end{hint}
    \end{column}
  \end{columns}
...
final addTodo = FunctionDeclaration(
  'add_todo',
  'Create a todo for the signed-in user. Use only when the '
  'user asks to add, create or remember a task.',
  parameters: {
    'title': Schema.string(description: 'Short task text'),
  },
);
final model = FirebaseAI.googleAI().generativeModel(
  model: 'gemini-2.5-flash',
...
\end{dart}
  \muted{The second tool wraps the location call from Lecture 11. The runtime permission is still the user's decision, and a refusal is an ordinary tool result to report back, not an exception to throw.}
  \note{Note that the declaration says nothing about how the tool is implemented. That separation is what lets the same declaration point at a local function today and at a backend endpoint tomorrow.}
\end{frame}
...
while (res.functionCalls.isNotEmpty && rounds++ < 4) {
...
return res.text ?? '';
\end{dart}
  \muted{The model asks, your app runs, the result goes back as another turn, and the model answers using it. Everything called an agent is this loop with a stop condition bolted on.}
...
\begin{frame}[eyebrow=Tools]{Parallel calls, stop conditions, agents}
  ...
  \end{grid}
  \vskip 3mm
  \muted{Give the model a way to finish: a tool that answers, an explicit instruction to reply in text when it has enough, and a final turn where you send the tool results with no tools attached.}
...
\begin{frame}[eyebrow=In the app]{Where every piece runs}
  ...
  {\usebeamerfont{small}\renewcommand{\arraystretch}{1.05}\begin{tabular}{@{}lp{4.4cm}p{4.8cm}@{}}
    ...
    \midrule
    Provider key & your backend, or Firebase AI Logic & a key inside a binary is a published key \\
    Attestation & App Check, in enforced mode & only your build may spend your quota \\
  ...
  \begin{takeaway}{Nothing here is new since Lecture 12.}
    The credential rules that governed a single call govern a whole agent loop, with more surface.
  \end{takeaway}
...
func addTodo(w http.ResponseWriter, r *http.Request) {
    ...
    // uid comes from the verified token, never from an argument
    writeTodo(r.Context(), uid, in.Title)
}
\end{golang}
  \muted{The model supplies the title and nothing else. If it could name the owner, one poisoned note would be enough to write into somebody else's account.}
...
Future<void> ask(String q) async {
  await _sub?.cancel();   // new question wins
  emit(const Thinking());
...
\end{dart}
    ...
    \begin{column}{0.41\textwidth}
      \lead{Four states, one BlocBuilder.}{Idle, thinking, writing, failed. A spinner that never changes reads as a hang.}
      \vskip 1mm
      \muted{Retrieval happens before the first token, so show that step too. Cancel the subscription in close(): a stream left running after the screen is gone still costs tokens.}
    \end{column}
  \end{columns}
