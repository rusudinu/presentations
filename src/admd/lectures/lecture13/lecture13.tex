% ADMD Lecture 13. Compile: latexmk -xelatex lecture13.tex
...
\end{dart}
  ...
  \end{takeaway}
  \note{Point at the seed: it is the only color the designer picks. Show that the generated palette is not the seed repeated, it is a set of tones. Say the theme lives in one file, for example lib/theme/app\_theme.dart.}
\end{frame}
...
\end{shell}
  ...
  \begin{takeaway}{Green on your laptop proves nothing.}
    Run analyze and test on every pull request. A branch that fails the checks cannot be merged. The pipeline itself is Lecture 14.
  \end{takeaway}
  \note{Say that the CI job for this course runs the first two commands on every pull request, and that the lab grade depends on them passing. Integration tests need a device, so they usually run on a schedule rather than on every push.}
\end{frame}
