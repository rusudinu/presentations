...
\objectivesframe{
  \cell[\faCubes]{LLM fundamentals}{Tokens, embeddings, attention, the context window, and why a wrong answer sounds like a right one.}
  \cell[\faDatabase]{Retrieval-augmented generation}{Chunk, embed, retrieve, cite. Build the retrieve-augment-generate loop and know how it fails.}
  \cell[\faWrench]{Tool calling}{Declare a tool, run the call loop, confirm before a side effect, and read a Model Context Protocol server.}
  \cell[\faMobile]{Flutter integration}{Where each piece runs, streaming into a bloc, cost caps, logging without leaking, evaluation before release.}
}

\divider{Part 1 · The model}{Large language models}[Tokens, attention, context, hallucination]

\begin{frame}[eyebrow=Model]{Tokens}
  \vskip 2mm
  \begin{grid}[3]
  ...
  \begin{takeaway}{The context limit, the latency and the bill are all counted in tokens.}
  \end{takeaway}
...
\begin{frame}[eyebrow=Model]{Embeddings}
  \begin{columns}[T]
    ...
    \end{column}
  \end{columns}
  \begin{takeaway}{Embeddings are what make search over your own documents possible.}
    Part 3 builds on them.
  \end{takeaway}
...
\begin{frame}[eyebrow=Model]{Attention and next-token prediction}
  \begin{columns}[T]
...
\begin{frame}[eyebrow=Model]{The context window}
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
...
\begin{frame}[fragile,eyebrow=Model]{Sampling and temperature}
  \begin{columns}[T]
...
\begin{frame}[eyebrow=Model]{Deployment options and cost}
  \vskip 2mm
  \begin{grid}[3]
...
\divider{Part 2 · Prompting}{Prompt design}[System prompt, examples, schemas, and injection]

\begin{frame}[eyebrow=Prompting]{Prompt structure}
  \vskip 2mm
  \begin{grid}[2]
  ...
  \end{grid}
  \begin{takeaway}{Treat a prompt as source code.}
...
\begin{frame}[fragile,eyebrow=Prompting]{Structured output with a schema}
\begin{dart}
...
\begin{frame}[fragile,eyebrow=Prompting]{Prompt injection}
  \begin{columns}[T]
    \begin{column}{0.53\textwidth}
\begin{codeblock}
...
\begin{frame}[eyebrow=Prompting]{Prompt injection mitigations}
  \vskip 1mm
  \begin{grid}[2]
...
\divider{Part 3 · Retrieval}{Retrieval-augmented generation}[Similarity search, chunking, vector storage, grounding]

\begin{frame}[eyebrow=Retrieval]{Limits of model knowledge}
  \vskip 2mm
  \begin{grid}[3]
  ...
  \begin{takeaway}{RAG is search plus prompt assembly plus a grounding rule, all in your code.}
  \end{takeaway}
...
\begin{frame}[eyebrow=Retrieval]{Similarity search}
  \begin{columns}[T]
    ...
    \begin{column}{0.44\textwidth}
      ...
      \vskip 1mm
      \muted{Exact search compares every vector and is always right. Approximate nearest neighbor search trades a little recall for speed, which is what production stores do.}
    \end{column}
  \end{columns}
...
\begin{frame}[eyebrow=Retrieval]{Chunking}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
...
\end{dart}
  \muted{The system prompt carries the grounding rule: answer only from the context, say you do not know when it is missing, cite the source labels. A shared index queried without the owner filter is a data leak.}
...
\begin{frame}[eyebrow=Retrieval]{Grounding checks}
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      \begin{enumerate}
...
\divider{Part 4 · Tools}{Tool calling}[Function declarations, the call loop, and MCP]

\begin{frame}[eyebrow=Tools]{Tool declarations}
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
    ...
    \begin{column}{0.42\textwidth}
      \begin{hint}[Keep the set small]
        Every declaration costs input tokens on each turn and gives the model one more thing to confuse.
      \end{hint}
    \end{column}
  \end{columns}
  \begin{takeaway}{A tool the model never picks, or picks for the wrong question, usually has a poor description.}
  \end{takeaway}
...
\begin{frame}[eyebrow=Tools]{Parallel calls, stop conditions, agents}
  ...
  \begin{takeaway}{Every production loop has a maximum number of rounds and a wall-clock deadline.}
  \end{takeaway}
...
\begin{frame}[eyebrow=Tools]{Confirmation before side effects}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \lead{Split read tools from write tools.}{A read tool can run freely. A tool that writes, spends, deletes or messages another person stops and asks first.}
      \vskip 1mm
    ...
    \end{column}
  \end{columns}
  \begin{takeaway}{A write that can be undone within ten seconds protects better than a confirmation dialog.}
  \end{takeaway}
...
\begin{frame}[eyebrow=Tools]{Model Context Protocol (MCP)}
  \vskip 2mm
  \begin{grid}[3]
  ...
  \begin{takeaway}{MCP standardizes tool discovery.}
    One server can serve your backend, an editor and a desktop assistant without a line of glue in each.
  \end{takeaway}
...
\divider{Part 5 · In the app}{Flutter app integration}[Architecture, streaming, cost, logging, evaluation]

\begin{frame}[eyebrow=In the app]{System architecture}
  \vskip 1mm
...
\begin{frame}[fragile,eyebrow=In the app]{Backend tool endpoint}
\begin{golang}
...
\begin{frame}[eyebrow=In the app]{Assistant tab walkthrough}
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      \begin{enumerate}
