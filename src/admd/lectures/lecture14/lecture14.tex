...
\objectivesframe{
  \cell[\faPalette]{Theming and dark mode}{Build the app's colors and type once with ThemeData, and get dark mode for free.}
  \cell[\faKeyboard]{Forms}{Validators, focus order, keyboard types, and error messages a user can act on.}
  \cell[\faUniversalAccess]{Accessibility and localization}{Semantics, contrast, tap targets, text scaling, and localized strings.}
  \cell[\faVial]{Testing}{Unit, widget, bloc, golden and integration tests, all running on every pull request.}
}

\divider{Part 1 · Theming}{Theming}[ThemeData, Material 3, dark mode, and responsive layout]

\begin{frame}[fragile,eyebrow=Theming]{Hard-coded colors}
  \begin{columns}[T]
...
\end{dart}
    ...
    \begin{column}{0.41\textwidth}
      ...
      \muted{It is also unsearchable: finding which blue the button uses means reading forty files.}
    \end{column}
  \end{columns}
  \begin{takeaway}{A widget that names a role such as \code{cs.primary} keeps working when the palette, the mode or the brand changes.}
  \end{takeaway}
...
\begin{frame}[eyebrow=Theming]{Common color roles}
  \vskip 1mm
...
\begin{frame}[fragile,eyebrow=Theming]{Dark mode}
\begin{dart}
MaterialApp(
...
\begin{frame}[fragile,eyebrow=Layout]{MediaQuery}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
\begin{dart}
...
\divider{Part 2 · Forms}{Forms}[Form, TextFormField, validators, focus, and error display]

\begin{frame}[fragile,eyebrow=Forms]{Flutter form structure}
\begin{dart}
...
\begin{frame}[fragile,eyebrow=Forms]{Form validation}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
\begin{dart}
...
\begin{frame}[eyebrow=Forms]{Error display}
  \vskip 2mm
  \begin{grid}[3]
    \cell[\faExclamationCircle]{Errors under the field}{The message sits where the mistake is.}
    ...
    \cell[\faUndo]{Keep what they typed}{On failure, keep every field.}
...
\divider{Part 3 · Motion}{Implicit animations}[Implicit animations, Hero, duration, reduced motion]

\begin{frame}[eyebrow=Motion]{Implicit animation widgets}
  \vskip 2mm
  \begin{grid}[3]
...
\begin{frame}[fragile,eyebrow=Motion]{AnimatedContainer and AnimatedSwitcher}
\begin{dart}
...
\begin{frame}[fragile,eyebrow=Motion]{Hero transitions}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
\begin{dart}
// List screen
...
\begin{frame}[fragile,eyebrow=Motion]{Animation duration and reduced motion}
\begin{dart}
...
\end{dart}
  ...
  \begin{tabular}{@{}lp{48mm}p{58mm}@{}}
    ...
    \bottomrule
  \end{tabular}
  \begin{takeaway}{If the user notices the animation instead of the result, it is too long.}
Never block input until it ends.
  \end{takeaway}
...
\begin{frame}[eyebrow=Access]{Common accessibility barriers}
  \vskip 2mm
  \begin{grid}[3]
    ...
    \cell[\faHandPointer]{Tiny targets}{A 24 dp icon is a 24 dp target unless you say otherwise.}
...
\begin{frame}[fragile,eyebrow=Access]{Semantics and screen readers}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
\begin{dart}
IconButton(
...
\end{dart}
    ...
    \end{column}
  \end{columns}
  \begin{takeaway}{Flutter builds a semantics tree on its own.}
    Add \code{Semantics} wherever the pixels do not carry the meaning.
  \end{takeaway}
...
\begin{frame}[eyebrow=Access]{Contrast, tap targets, and text scaling}
  \vskip 1mm
  ...
  \begin{takeaway}{Test with the phone's own settings.}
    Set the font size to the largest step, turn a screen reader on, and open every screen.
  \end{takeaway}
...
\begin{frame}[fragile,eyebrow=Locale]{Localized strings in ARB files}
\begin{yaml}
# pubspec.yaml
flutter:
  generate: true
...
\begin{frame}[fragile,eyebrow=Locale]{Plurals, dates, and right-to-left layout}
\begin{codeblock}
// lib/l10n/app_en.arb
...
\divider{Part 5 · Testing}{Testing}[Unit, widget, bloc, golden and integration tests]

\begin{frame}[eyebrow=Testing]{The testing pyramid}
  \vskip 1mm
...
\begin{frame}[fragile,eyebrow=Testing]{Unit tests}
\begin{dart}
...
\begin{frame}[fragile,eyebrow=Testing]{pump and pumpAndSettle}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
\begin{dart}
await tester.pump();
...
\end{dart}
    \end{column}
    \begin{column}{0.41\textwidth}
      \lead{Test time does not pass on its own.}{Nothing animates, no timer fires and no Future completes until you pump, so the test is deterministic.}
      \begin{warning}
...
\begin{frame}[fragile,eyebrow=Testing]{Mocking with mocktail}
\begin{dart}
...
\begin{frame}[fragile,eyebrow=Testing]{Bloc tests}
\begin{dart}
...
\begin{frame}[fragile,eyebrow=Testing]{Golden tests}
\begin{dart}
...
\end{dart}
  ...
  \begin{takeaway}{Commit the golden files.}
    The diff in the review is a picture, so a designer can review it.
  \end{takeaway}
...
\begin{frame}[fragile,eyebrow=Testing]{integration\_test and CI}
\begin{shell}
flutter analyze
...
\end{shell}
  \vskip 2mm
  \muted{An integration test starts with \code{IntegrationTestWidgetsFlutterBinding.ensureInitialized()} and then uses the same \code{testWidgets} API, except that the full app runs on a device with its plugins.}
  \begin{takeaway}{Run analyze and test on every pull request.}
    A branch that fails the checks cannot be merged. The pipeline itself is Lecture 15.
  \end{takeaway}
  \note{Say that the CI job for this course runs the first two commands on every pull request. Integration tests need a device, so they usually run on a schedule rather than on every push. The integration\_test package comes from the SDK: add it under dev\_dependencies as integration\_test with sdk: flutter, never from pub.dev.}
\end{frame}
...
\begin{frame}[eyebrow=Recap]{Summary}
  ...
  \begin{enumerate}
    ...
    \item Keep animations short and error messages specific. \muted{Use a 48 dp tap target, and a layout that adapts when the window is twice as wide.}
