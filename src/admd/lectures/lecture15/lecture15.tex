...
\objectivesframe{
  \cell[\faBoxOpen]{Release builds}{Debug, profile and release, flavors, icons, and a version number the stores accept.}
  \cell[\faKey]{App signing}{Android keystores and Play App Signing, iOS certificates and provisioning profiles.}
  \cell[\faRocket]{Store delivery and CI/CD}{Play Console and App Store Connect, internal testing and TestFlight, GitHub Actions and fastlane.}
  \cell[\faBell]{Push notifications}{FCM tokens, the three app states, and what a notification payload may carry.}
}

\divider{Part 1 · Building}{Release builds}[Build modes, flavors, icons, and version numbers]

\begin{frame}[eyebrow=Building]{Build modes}
  \vskip 2mm
  \begin{grid}[3]
...
\begin{frame}[fragile,eyebrow=Building]{Store build artifacts}
  \begin{columns}[T]
...
\begin{frame}[fragile,eyebrow=Building]{Flavors}
  \begin{columns}[T]
...
\divider{Part 2 · Signing}{App signing}[Keystores, Play App Signing, certificates and profiles]
...
\begin{frame}[eyebrow=Signing]{iOS certificates and provisioning profiles}
...
\begin{frame}[eyebrow=Signing]{Keeping secrets out of the repository}
  \vskip 2mm
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      ...
      \muted{If it happens: rotate the credential first, then clean the history. Rotation is what stops the damage.}
...
\divider{Part 3 · Stores}{Store submission and review}[Play Console, App Store Connect, test tracks, and privacy]
...
\begin{frame}[eyebrow=Stores]{Google Play Console: the first release}
  ...
  \end{enumerate}
  \vskip 3mm
  \muted{Review takes days, and a new developer account waits longest.}
  \note{The realistic message: the binary is the easy half. Budget an afternoon for the forms and screenshots, and do it before the deadline, not on it. Mention the one-time 25 dollar registration fee, and that since August 2026 a new upload must target API level 36 and its 64-bit code must support 16 KB memory pages.}
\end{frame}
...
\begin{frame}[eyebrow=Stores]{App Store Connect: the first release}
  ...
  \note{Say that Apple's review is a human reading your notes and tapping through the app. Everything you would tell a new tester belongs in the review notes field. Membership costs 99 dollars a year, and since April 2026 every upload must be built against the iOS 26 SDK, which means Xcode 26 or newer.}
\end{frame}

\begin{frame}[eyebrow=Stores]{Test tracks}
  \vskip 1mm
  ...
  \end{takeaway}
  \note{Point out that TestFlight builds expire after 90 days, so a beta is not a distribution channel, and that it allows 100 internal testers and up to 10,000 external ones. A personal Play account registered after November 2023 must also hold 12 testers on closed testing for 14 continuous days before it may publish to production. Internal testing on Play is the fastest loop you have.}
\end{frame}

\begin{frame}[eyebrow=Stores]{Common rejection reasons}
  \vskip 1mm
  \begin{grid}[3]
...
\divider{Part 4 · Automation}{Release automation}[GitHub Actions, fastlane, Codemagic, and code push]

\begin{frame}[eyebrow=Automation]{Release pipeline stages}
  \vskip 2mm
  \begin{grid}[3]
  ...
  \end{grid}
  \vskip 4mm
  \muted{The pipeline builds from a clean checkout every time, with no local caches and no local keys.}
  \begin{takeaway}{Automate tests and analysis on pull requests first.}
    Automated store upload can wait until you release often.
  \end{takeaway}
...
jobs:
  test:
    ...
    steps:
      ...
      - run: flutter test
\end{yaml}
  \muted{Format, analyze and test are the same commands you run locally.}
...
\end{yaml}
    ...
    \end{column}
  \end{columns}
  \begin{takeaway}{Rotate any secret that was printed in a build log.}
    Deleting the log does not help.
  \end{takeaway}
...
\begin{frame}[eyebrow=Automation]{Code push}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      ...
      \muted{It covers Dart code and assets. Anything native, a new plugin, a new permission, or a version bump still needs a store release.}
    \end{column}
    \begin{column}{0.46\textwidth}
...
\divider{Part 5 · Push}{Push notifications}[FCM, APNs, tokens, and the three app states]

\begin{frame}[eyebrow=Push]{Push delivery path}
  \vskip 2mm
  \begin{grid}[3]
  ...
  \begin{takeaway}{Send an identifier in the push and let the app fetch the data over HTTP.}
    Never treat a push as your only copy of a state change.
  \end{takeaway}
...
\end{dart}
    ...
    \end{column}
  \end{columns}
  \note{Make them test all three by hand: app open, app backgrounded, app force stopped. Almost every push bug reported in the labs is one of the three cases never being tried. Name flutter\_local\_notifications as the package that draws the foreground notification.}
\end{frame}

\begin{frame}[eyebrow=Push]{Notification permission and content}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \lead{You get one prompt.}{iOS has always required permission, and Android 13 added POST\_NOTIFICATIONS. A denial is final until the user opens system settings.}
      ...
      \muted{Group related messages, collapse duplicates, and respect quiet hours.}
    \end{column}
    \begin{column}{0.48\textwidth}
...
\begin{frame}[eyebrow=Checklist]{Release checklist}
  \vskip 2mm
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{checklist}
...
\begin{frame}[eyebrow=Course recap]{Course overview}
  \vskip 1mm
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
...
\begin{frame}[eyebrow=Lab test]{Lab test and grading}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      ...
      \muted{7.5 points per requirement, partial credit for a requirement that is present but incomplete. Submit on branch \code{lab-7} before the end.}
    \end{column}
    \begin{column}{0.46\textwidth}
...
\begin{frame}[eyebrow=Recap]{Summary}
  \vskip 2mm
  \begin{enumerate}
    \item A release includes signing, versioning, store forms and review. \muted{Together they take longer than the compile.}
    \item Guard the keys, automate the rest. \muted{A lost signing key is unrecoverable. Everything else can be scripted.}
    \item A push carries an identifier; the app fetches the content. \muted{Never put a secret in a payload.}
  \end{enumerate}
  \vskip 5mm
