...
\subtitle{TinyGo, a blinking LED, a local MQTT broker, team setup}
...
\begin{frame}[fragile,eyebrow=Setup]{Repository and branch}
  \begin{steps}
...
\end{shell}
    \item Open a pull request from \code{lab-1} on the first commit.
  \end{steps}
  \begin{hint}
...
\begin{frame}[fragile,embedded,eyebrow=Task I]{Board and TinyGo target}
  \begin{columns}[T]
...
\begin{frame}[fragile,embedded,eyebrow=Reference]{Blink program: full listing}
\begin{golang}
package main
import "machine"
import "time"
...
\begin{frame}[fragile,embedded,eyebrow=Task III]{Wokwi simulation without a board}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{steps}
...
\begin{frame}[eyebrow=Task V]{Team and project idea}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{steps}
...
\begin{frame}[eyebrow=Reference]{Project requirements}
  {\usebeamerfont{small}
...
\begin{frame}[embedded,eyebrow=Troubleshooting]{Flashing and serial port errors}
  \vskip 2mm
...
\begin{frame}[eyebrow=Troubleshooting]{Broker connection errors}
  \vskip 2mm
...
\begin{frame}[eyebrow=GitHub]{Pull request contents}
  \vskip 2mm
  \begin{grid}[2]
...
\closingframe{Submission}{Branch lab-1 · blink and serial output in the PR · idea approved}
