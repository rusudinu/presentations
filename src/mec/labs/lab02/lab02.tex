...
\subtitle{I2C sensor in TinyGo, MQTT publishing, live value in Flutter}
...
\begin{frame}[embedded,eyebrow=Setup]{Hardware setups and message contract}
  \vskip 1mm
  \begin{grid}[2]
...
\begin{frame}[fragile,eyebrow=Setup]{Branch, dependencies, file layout}
  \begin{steps}
    \item Branch the team repository and add the dependencies:
\begin{shell}
git checkout -b lab-2
cd device && go get tinygo.org/x/drivers github.com/soypat/natiu-mqtt
cd ../app && flutter pub add mqtt_client
\end{shell}
    \item Firmware in \code{device/lab2/main.go}. App code in \code{lib/lab2/}.
...
\begin{frame}[embedded,fragile,eyebrow=Task I]{Sensor loop}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
\begin{golang}
...
\begin{frame}[embedded,fragile,eyebrow=Task I]{Simulated reading without a sensor}
  \vskip 1mm
\begin{golang}
...
\end{golang}
  ...
  \end{columns}
  \vskip 1mm
  \muted{Publish a value that moves, so the number on the phone shows the chain is alive.}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{steps}
        ...
        \item Print the result of every step.
      \end{steps}
    \end{column}
    \begin{column}{0.41\textwidth}
      \kv{Done when}{}
...
\begin{frame}[embedded,fragile,eyebrow=Task II]{Wi-Fi and broker connection}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
\begin{golang}
lk := link.Esplink{}   // espradio
netdev.UseNetdev(&lk)
lk.NetConnect(&nl.ConnectParams{
    Ssid: ssid, Passphrase: pass})

conn, _ := net.Dial("tcp", broker)
var q mqtt.OrchestratorNoAlloc
q.Configure(qcfg)   // QoS 1 state
c := mqtt.NewClient(mqtt.ClientConfig{
    Orchestrator: &q})
var v mqtt.VariablesConnect
v.SetDefaultMQTT([]byte(id))
c.Connect(ctx, conn, &v)
\end{golang}
    \end{column}
    \begin{column}{0.41\textwidth}
      \lead{Treat this as a shape.}{The radio is \code{tinygo.org/x/espradio}, the client is \code{natiu-mqtt}. Keep the sequence: join, dial, connect, publish, check the error each time.}
      \begin{hint}[The address]
        \code{broker} is your laptop's address and port on the lab network, such as \code{192.168.1.10:1883}. Not \code{localhost}: that is the board.
      \end{hint}
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}[embedded,fragile,eyebrow=Task II]{Topic and payload}
  \vskip 1mm
\begin{golang}
topic := "course/" + team + "/reading"          // one team, one topic
p := fmt.Sprintf(`{"team":%q,"c":%.2f,"seq":%d}`, team, celsius, seq)
fl, _ := mqtt.NewPublishFlags(mqtt.QoS1, false, false)   // retain off
pv := mqtt.VariablesPublish{TopicName: []byte(topic)}
c.PublishPayload(fl, pv, []byte(p))  // then HandleNext for the PUBACK
time.Sleep(2 * time.Second)
\end{golang}
  \vskip 2mm
  \begin{columns}[T]
...
\begin{frame}[embedded,fragile,eyebrow=Task II]{Pico without Wi-Fi: laptop bridge}
  \vskip 1mm
\begin{shell}
...
\end{shell}
  \vskip 1mm
  \muted{The board prints the JSON document, one per line, as a networked board would publish it. The laptop only forwards. Nothing else in the lab changes: the phone cannot tell the two set-ups apart.}
  \vskip 2mm
  \begin{warning}
...
\begin{frame}[fragile,eyebrow=Task III]{Rendering and reconnecting}
\begin{dart}
...
\end{dart}
  \vskip 1mm
  \muted{Two clients with the same id disconnect each other, which looks like a broken network.}
\end{frame}

\begin{frame}[fragile,eyebrow=Acceptance]{Acceptance checks}
  \vskip 1mm
\begin{shell}
...
\end{shell}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      ...
      One commit per task. A screenshot of the app next to the subscriber terminal.}
    \end{column}
  \end{columns}
\end{frame}
...
\closingframe{Submission}{Branch lab-2 · one commit per task · your topic in the README}
