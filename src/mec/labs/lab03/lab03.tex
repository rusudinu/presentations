...
\subtitle{BLE peripheral in Go, Flutter scan and connect, reconnection}
...
\begin{frame}[embedded,fragile,eyebrow=Peripheral]{Starting the peripheral}
  \begin{columns}[T]
...
\begin{frame}[embedded,fragile,eyebrow=Reference]{Peripheral, part 1: advertising}
\begin{golang}
package main
import "time"
...
\begin{frame}[embedded,fragile,eyebrow=Reference]{Peripheral, part 2: notifications}
\begin{golang}
    must(adv.Start())
...
\begin{frame}[eyebrow=Task I]{Permissions and device scan}
  \begin{columns}[T]
...
\begin{frame}[fragile,eyebrow=Reference]{Manifest and Info.plist entries}
\begin{codeblock}
...
\begin{frame}[eyebrow=Task III]{Reconnection after a disconnect}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{steps}
...
\begin{frame}[fragile,eyebrow=Reference]{Reconnect loop}
\begin{dart}
...
\begin{frame}[eyebrow=Troubleshooting]{No values received}
  \vskip 2mm
...
\begin{frame}[embedded,eyebrow=Troubleshooting]{Peripheral errors}
...
\begin{frame}[eyebrow=Submission]{Acceptance checks}
  \vskip 2mm
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
    ...
    \begin{column}{0.44\textwidth}
      \kv{How to submit}{}
      \muted{Branch \code{lab-3} in the team repository, one commit per task, pushed before the next lab. Put your two UUIDs and the byte encoding in the README.}
      \vskip 2mm
...
\closingframe{Submission}{Branch lab-3 · peripheral, scan, live value, reconnect}
