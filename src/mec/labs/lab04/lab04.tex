...
\subtitle{Step detection, location, background tasks}
...
\begin{frame}[embedded,eyebrow=Task I]{Step counter from raw acceleration}
  \begin{columns}[T]
...
\begin{frame}[embedded,fragile,eyebrow=Task I]{Threshold and debounce: code}
\begin{dart}
...
\begin{frame}[eyebrow=Task II]{Location and the permission flow}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{steps}
...
\begin{frame}[fragile,eyebrow=Task II]{Permission declarations}
...
\begin{frame}[eyebrow=Task II]{Precise and approximate location}
  \vskip 2mm
  \begin{columns}[T]
...
\begin{frame}[eyebrow=Task III]{Background task}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{steps}
...
\begin{frame}[fragile,eyebrow=Task III]{Background task callback}
\begin{dart}
const _key = 'lab4_log';
...
\begin{frame}[eyebrow=Task III]{Background execution: Android and iOS}
  \vskip 1mm
...
\begin{frame}[eyebrow=Acceptance]{Acceptance checks}
  ...
  \end{checklist}
  \vskip 3mm
  \begin{takeaway}{Put one screenshot per task in the pull request.}
    The timestamp list, a shake in progress, and a position with its accuracy.
  \end{takeaway}
\end{frame}
...
\closingframe{Submission}{Branch lab-4 · one commit per task · a screenshot of the timestamp list in the PR description}
