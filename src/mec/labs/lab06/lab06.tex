...
\subtitle{gRPC unary call, server stream, deadlines, cancellation}
...
\begin{frame}[fragile,eyebrow=Setup]{Contract: readings.proto}
\begin{codeblock}
syntax = "proto3";
package readings.v1;
message Reading {
...
\begin{frame}[fragile,eyebrow=Task I]{Generate the stubs, run the server}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{steps}
        \item Run protoc for Go, then for Dart (the commands are next).
...
\begin{frame}[fragile,eyebrow=Task I]{Go server: struct and GetLatest}
  \begin{columns}[T]
...
\begin{frame}[fragile,eyebrow=Task I]{Go server: stream and main}
  \begin{columns}[T]
...
\begin{frame}[fragile,eyebrow=Task II]{Unary call with a deadline}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{steps}
...
\begin{frame}[fragile,eyebrow=Task II]{Dart client: unary call}
  \begin{columns}[T]
    \begin{column}{0.6\textwidth}
\begin{dart}
...
\begin{frame}[fragile,eyebrow=Task III]{Streaming screen with cancel}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{steps}
...
\begin{frame}[fragile,eyebrow=Task III]{Dart client: stream and cancel}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
\begin{dart}
...
\closingframe{Submission}{Branch lab-6 · one commit per task · README with device id scheme and port}
