...
\decklabel{Lab 7}
\title{Lab test}
\subtitle{Rules, format, two practice specs}
...
\begin{frame}[eyebrow=Today]{Lab objectives}
  ...
  \begin{grid}[2]
    ...
    \cell[\faIcon{graduation-cap}]{Worth 15 points}{5 points per requirement, graded from your pushed branch. There is no live demo.}
  \end{grid}
  \vskip 6mm
...
\begin{frame}[eyebrow=Rules]{Rules}
  ...
  \begin{takeaway}{The clock does not pause.}
    A crashed simulator or a broker that refuses a connection counts against your ninety minutes.
  \end{takeaway}
\end{frame}
...
\begin{frame}[embedded,eyebrow=Format]{Requirement types}
  \begin{grid}[3]
...
\begin{frame}[eyebrow=Rubric]{Rubric}
  {\usebeamerfont{small}\begin{tabular}{@{}p{30mm}p{42mm}p{42mm}@{}}
    \toprule
    \thead{Requirement} & \thead{Full credit (5 p)} & \thead{Partial credit (2.5 p)} \\
    \midrule
