...
\subtitle{Course plan, device spectrum, and code execution}
...
\objectivesframe{
  \cell[\faGraduationCap]{Course plan}{The 14 lectures, the 7 labs, the 7 project sessions, and how the 10 points are earned.}
  \cell[\faIcon{mobile-alt}]{Device spectrum}{Where a phone, a single-board computer and a microcontroller sit, and how their budgets differ.}
  \cell[\faCode]{Compilation models}{Compilers, interpreters, JIT and AOT, and why Dart is built both ways.}
  \cell[\faRecycle]{Memory on a phone}{What a garbage collector does for you, and what it costs inside a frame.}
}

% =====================================================================
\divider{Part 1 · Orientation}{Course organization}[Lectures, labs, project, grading]
...
\begin{frame}[eyebrow=Orientation]{Instructors}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.46\textwidth}
      ...
      \vskip 4mm
      \kv{Course repository}{\link{https://github.com/rusudinu/presentations}{github.com/rusudinu/presentations}}
    \end{column}
...
\begin{frame}[eyebrow=Orientation]{Course scope}
  \vskip 2mm
  \begin{grid}[3]
...
\begin{frame}[eyebrow=Orientation]{Labs and project sessions}
  \vskip 1mm
  {\usebeamerfont{small}
  \begin{tabular}{@{}rll@{}}
    \toprule
...
\begin{frame}[eyebrow=Orientation]{Grading}
  \vskip 4mm
  \begin{grid}[3]
    \bignum{40 p}{final exam, in the exam session}
    \bignum{30 p}{laboratory and assignments}
    \bignum{15 p}{lab test, in week 14}
    \bignum{15 p}{team project, teams of at most 3}
    \bignum{50}{points needed to pass}
  \end{grid}
  \begin{takeaway}{Sixty of the hundred points come from work done during the semester.}
  \end{takeaway}
...
\begin{frame}[eyebrow=Orientation]{Concept presentations}
  ...
  \end{takeaway}
  \note{Open the shared sheet on the projector and show that claiming is first come, first served.}
\end{frame}

\begin{frame}[eyebrow=Orientation]{Project requirements}
  \vskip 2mm
  \begin{grid}[3]
...
\begin{frame}[eyebrow=Orientation]{Project milestones}
  \vskip 2mm
  {\usebeamerfont{small}
  \begin{tabular}{@{}rl@{}}
    \toprule
...
\begin{frame}[eyebrow=Orientation]{Prerequisites}
  \vskip 2mm
  \begin{columns}[T]
...
% =====================================================================
\divider{Part 2 · The device spectrum}{From the cloud to a microcontroller}

\begin{frame}[eyebrow=Device spectrum]{The device spectrum}
  \vskip 2mm
  {\usebeamerfont{small}
  ...
  \begin{takeaway}{This course lives on the right half of the table.}
    Board means a single-board computer, MCU a microcontroller.
  \end{takeaway}
...
\begin{frame}[eyebrow=Device spectrum]{Mobile and embedded constraints}
  \vskip 2mm
  \begin{grid}[3]
...
\begin{frame}[eyebrow=Device spectrum]{Device resources}
  \vskip 1mm
  \begin{grid}[3]
  ...
  \begin{takeaway}{On a desktop the first four are effectively elastic.}
    On a phone all six are hard budgets.
  \end{takeaway}
...
\begin{frame}[eyebrow=Device spectrum]{Key figures}
  \vskip 2mm
  \begin{grid}[3]
  ...
  \begin{takeaway}{One frame budget equals one divided by the refresh rate.}
    Most phones sold in 2026 run at 90–120 Hz, so the familiar 16 ms describes a 60 Hz screen.
  \end{takeaway}
...
% =====================================================================
\divider{Part 3 · Embedded}{Embedded systems and the phone as hub}[Kilobytes of RAM, milliwatts of power, no operating system]

\begin{frame}[embedded,eyebrow=Embedded]{Embedded constraints}
  \vskip 2mm
  \begin{columns}[T]
    ...
    \begin{column}{0.42\textwidth}
      \begin{hint}[Boards for this course]
        TinyGo supports the ESP32-C3 and ESP32-S3, with Wi-Fi and Bluetooth through the espradio package, and the Raspberry Pi Pico. The original ESP32 gets Wi-Fi, but no Bluetooth.
      \end{hint}
    \end{column}
  \end{columns}
  ...
  \end{takeaway}
  \note{Say the board recommendation plainly and repeat it in Lab 1. Steer students away from the original Xtensa ESP32: TinyGo now drives it and espradio gives it Wi-Fi, but there is still no Bluetooth from TinyGo on that chip, and Lab 3 needs BLE.}
\end{frame}

\begin{frame}[embedded,eyebrow=Embedded]{The phone as hub}
  \vskip 1mm
  \begin{grid}[3]
...
% =====================================================================
\divider{Part 4 · How code runs}{Code execution}[Compilers, interpreters, JIT, AOT, and garbage collection]

\begin{frame}[eyebrow=How code runs]{Compilation stages}
  \vskip 2mm
  \begin{grid}[2]
  ...
  \end{grid}
  \begin{takeaway}{Toolchains differ in when step 3 happens.}
...
\begin{frame}[eyebrow=How code runs]{Compiler and interpreter}
  ...
  \begin{takeaway}{Compiled and interpreted describe a toolchain, not a language.}
    The same source can be run either way, and for Dart it is.
  \end{takeaway}
...
\begin{frame}[eyebrow=How code runs]{The compilation spectrum}
  ...
  \begin{tabular}{@{}lp{24mm}p{24mm}p{24mm}p{24mm}@{}}
    ...
    Examples   & Go, Rust, Swift & JVM, V8       & CPython & shell scripts \\
    \bottomrule
  \end{tabular}}
  \vskip 2mm
  \begin{takeaway}{Most runtimes mix these approaches.}
    Android's ART mixes all three: ahead-of-time compilation, a JIT, and profiles collected on the device.
  \end{takeaway}
...
\begin{frame}[eyebrow=How code runs]{Compiled versus interpreted performance}
  \vskip 2mm
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      \begin{itemize}
        \item There is a gap, and it depends on the workload. A tight numeric loop shows the largest difference. Code dominated by input and output shows almost none.
...
\begin{frame}[fragile,eyebrow=How code runs]{Dart JIT and AOT compilation}
  \vskip 2mm
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      \kv{Debug: JIT}{}
...
\end{shell}
      \muted{Compiled ahead of time to native ARM machine code. No compiler ships inside the app, only a small runtime. Fast cold start, smaller footprint. No hot reload, because there is nothing to reload into.}
    \end{column}
  \end{columns}
...
\begin{frame}[eyebrow=How code runs]{Garbage collection}
  \vskip 2mm
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      \begin{itemize}
...
\begin{frame}[eyebrow=Recap]{Summary}
  ...
  \begin{enumerate}
    ...
    \item Garbage collection costs frame time. \muted{A collection pause inside a frame budget is a dropped frame the user can see.}
  \end{enumerate}
  \vskip 5mm
