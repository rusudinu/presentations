...
\objectivesframe{
  \cell[\faClock]{Frame budget}{One frame is 1 \(\div\) refresh rate. Blocking the UI thread past that costs a dropped frame, not a crash.}
  \cell[\faIcon{random}]{Isolates for CPU work}{compute() for a one-shot call, Isolate.spawn for a worker that lives longer or streams results back.}
  \cell[\faIcon{share-alt}]{Goroutines and channels}{Go's concurrency primitives: go func(), typed channels, and select, on the server side of the same stack.}
  \cell[\faIcon{exchange-alt}]{Concurrency model comparison}{Dart isolates copy messages between heaps. Goroutines share one heap and rely on channels for discipline.}
}

\divider{Part 1 \textperiodcentered\ Foundations}{The frame budget and the event loop}[Event loop, microtasks, futures, and streams]

\begin{frame}[eyebrow=Foundations]{Frame budget}
  \vskip 2mm
  \begin{grid}[3]
...
\begin{frame}[fragile,eyebrow=Foundations]{Futures and async/await}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
\begin{dart}
...
\end{dart}
    ...
    \begin{column}{0.40\textwidth}
      ...
      \muted{A failure inside async code completes the Future with an error instead of throwing at the call site. \code{await} rethrows it, so try/catch behaves as it does in synchronous code.}
    \end{column}
  \end{columns}
...
\begin{frame}[fragile,eyebrow=Foundations]{Streams}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
\begin{dart}
...
\begin{frame}[fragile,eyebrow=Foundations]{Asynchrony versus parallelism}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
\begin{dart}
...
\divider{Part 2 \textperiodcentered\ Isolates}{CPU-bound work in isolates}

\begin{frame}[fragile,eyebrow=Isolates]{compute()}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
\begin{dart}
...
\begin{frame}[eyebrow=Isolates]{Async versus isolates}
  \vskip 1mm
...
\begin{frame}[fragile,eyebrow=Goroutines]{Go on the server side}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      \begin{itemize}
...
\begin{frame}[eyebrow=Goroutines]{Phone and server runtimes}
  \vskip 2mm
  \begin{grid}[2]
...
\end{golang}
    ...
    \begin{column}{0.40\textwidth}
      \begin{itemize}
        ...
        \item \code{handleReading} already runs concurrently for every caller.
...
\begin{frame}[fragile,eyebrow=Goroutines]{Goroutines}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
\begin{golang}
func main() {
...
\begin{frame}[fragile,eyebrow=Goroutines]{sync.WaitGroup}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
\begin{golang}
...
\begin{frame}[fragile,eyebrow=Goroutines]{Channels}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
\begin{golang}
...
\end{golang}
    ...
    \end{column}
  \end{columns}
  \begin{takeaway}{A buffered channel is a bounded queue.}
...
\begin{frame}[eyebrow=Goroutines]{Goroutine and channel pitfalls}
  ...
  \begin{takeaway}{None of this is checked for you.}
    The Dart isolate table two sections back had two rows of ``impossible''. Go has none.
  \end{takeaway}
...
\begin{frame}[fragile,eyebrow=Goroutines]{The select statement}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
\begin{golang}
select {
...
\end{golang}
    ...
    \end{column}
  \end{columns}
  \begin{takeaway}{select runs exactly one branch.}
  \end{takeaway}
...
\divider{Part 4 \textperiodcentered\ Compared}{Producer and consumer in Dart and Go}
...
\begin{frame}[eyebrow=Compared]{Dart isolates vs Go goroutines}
  ...
  {\usebeamerfont{small}\begin{tabular}{@{}p{24mm}p{53mm}p{53mm}@{}}
    ...
    \bottomrule
  \end{tabular}}
  \begin{takeaway}{Isolates copy every message; goroutines share one heap.}
    Isolates trade a copy per message for safety. Goroutines trade the copy for speed.
  \end{takeaway}
...
\begin{frame}[eyebrow=Compared]{Concurrency on the phone and the server}
  \vskip 2mm
  \begin{grid}[2]
