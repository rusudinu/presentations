...
\objectivesframe{
  \cell[\faMicrochip]{Datasheets}{Core, flash, SRAM, peripherals and clocks, and what those numbers forbid in your code.}
  \cell[\faBolt]{Execution models}{A super loop with interrupts and timers, or an RTOS with tasks. Know which one your project needs.}
  \cell[\faPlug]{Peripherals}{GPIO, ADC and PWM, and the difference between I2C, SPI and UART when you pick a sensor.}
  \cell[\faCode]{TinyGo}{Build, flash, read the serial console, and keep the device asleep between readings.}
}

\divider{Part 1 · Anatomy}{Microcontroller anatomy}

\begin{frame}[embedded,eyebrow=Anatomy]{Phone versus microcontroller resources}
  \vskip 1mm
...
\begin{frame}[embedded,eyebrow=Anatomy]{Microcontroller components}
  \vskip 2mm
  \begin{grid}[3]
...
\begin{frame}[embedded,eyebrow=Anatomy]{Memory constraints in practice}
  \vskip 1mm
  \begin{grid}[3]
    ...
    \cell[\faTrash]{Collection still happens}{The collector is small and simple. Allocating less is the only fix.}
  \end{grid}
...
\divider{Part 2 · Execution}{Bare metal and RTOS}[Super loop, interrupts, timers, RTOS]
...
\end{golang}
    ...
    \begin{column}{0.41\textwidth}
      \lead{There is nothing to return to.}{If \code{main} exits, the board stops or resets.}
...
\begin{frame}[embedded,eyebrow=Execution]{FreeRTOS}
  \vskip 1mm
  \begin{grid}[3]
...
\divider{Part 3 · Peripherals}{Peripheral interfaces}[GPIO, ADC, PWM, I2C, SPI, and UART]

\begin{frame}[embedded,eyebrow=Peripherals]{Common peripherals}
  \vskip 2mm
  \begin{grid}[3]
...
\begin{frame}[embedded,fragile,eyebrow=Peripherals]{GPIO}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
\begin{golang}
led := machine.LED
...
\begin{frame}[embedded,fragile,eyebrow=Peripherals]{Analog-to-digital conversion}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
\begin{golang}
machine.InitADC()
...
\end{golang}
    ...
    \begin{column}{0.41\textwidth}
      ...
      \muted{Readings jitter. Average several samples.}
...
\begin{frame}[embedded,fragile,eyebrow=Peripherals]{Pulse-width modulation}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
\begin{golang}
pwm := machine.PWM0
...
\begin{frame}[embedded,eyebrow=Peripherals]{I2C, SPI, and UART compared}
  \vskip 1mm
...
\divider{Part 4 · TinyGo}{Go on microcontrollers}[Toolchain, machine package, language limitations]

\begin{frame}[embedded,eyebrow=TinyGo]{TinyGo}
  \muted{TinyGo reads ordinary Go, emits LLVM intermediate code, and has LLVM produce machine code for Cortex-M, RISC-V, Xtensa, AVR and WebAssembly. It is a different compiler for the same language.}
  \vskip 2mm
  \begin{columns}[T]
...
\begin{frame}[embedded,fragile,eyebrow=TinyGo]{Board selection}
  \begin{columns}[T]
    \begin{column}{0.53\textwidth}
      \accenttext{\textbf{The original ESP32 has no Bluetooth here.}}
      \vskip 2mm
      \muted{The classic Xtensa ESP32, in most starter kits, is driven by TinyGo and gets Wi-Fi from espradio, but no Bluetooth.}
      \vskip 2mm
      \textbf{Buy one of these instead}
      \vskip 1.5mm
      \muted{ESP32-C3, a RISC-V part, or ESP32-S3, an Xtensa one: Wi-Fi from espradio, and Bluetooth since TinyGo 0.42.\par\vskip 1.5mm
...
\begin{frame}[embedded,fragile,eyebrow=TinyGo]{The machine package}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
\begin{golang}
func main() {
...
\end{golang}
    \end{column}
    \begin{column}{0.41\textwidth}
      \lead{One API, every board.}{\code{machine.LED} and \code{machine.ADC0} are aliases a board file defines. Where a target defines neither, name the pin yourself and the rest is unchanged.}
      \muted{\code{println} writes to the serial port. That is why the monitor is your debugger. An LED toggled in the loop shows that the loop still runs.}
    \end{column}
  \end{columns}
...
\end{golang}
    ...
    \begin{column}{0.41\textwidth}
      ...
      \muted{Do not write this by hand. The drivers repository carries over a hundred devices, each hiding the register map behind a typed reader.}
      \begin{hint}[Lab 2]
...
\begin{frame}[embedded,fragile,eyebrow=TinyGo]{Debugging over serial}
  \begin{columns}[T]
    \begin{column}{0.53\textwidth}
\begin{golang}
uart := machine.UART0
...
\end{golang}
    ...
    \end{column}
  \end{columns}
  \begin{takeaway}{Print a sequence number in every line.}
    It tells you whether the board stopped, reset, or lost its connection.
  \end{takeaway}
...
\begin{frame}[embedded,eyebrow=TinyGo]{TinyGo limitations}
  \vskip 1mm
  \begin{grid}[3]
...
\begin{frame}[embedded,fragile,eyebrow=TinyGo]{Goroutines in TinyGo}
  \begin{columns}[T]
    \begin{column}{0.53\textwidth}
\begin{golang}
...
\end{golang}
    ...
    \begin{column}{0.43\textwidth}
      ...
      \muted{Each goroutine costs a fixed stack, a kilobyte or more. Affordable for a handful, not for hundreds.}
...
\divider{Part 5 · Power}{Power management}[Sleep modes, wake sources, watchdog, Lab 1 setup]

\begin{frame}[embedded,eyebrow=Power]{Power consumption}
  \vskip 2mm
  \begin{grid}[3]
  ...
  \end{grid}
  \vskip 5mm
  \muted{Average current is roughly the active current times the active time, plus the sleep current times the sleep time, all divided by the period. Shorten the active window or lengthen the period.}
  \begin{takeaway}{Battery life depends on the fraction of time the radio is powered.}
  \end{takeaway}
...
\begin{frame}[embedded,fragile,eyebrow=Power]{Duty cycling}
  \begin{columns}[T]
    \begin{column}{0.53\textwidth}
\begin{golang}
for {
...
\end{golang}
    \end{column}
    \begin{column}{0.43\textwidth}
      \lead{Reconnecting costs less than staying connected.}{A keep-alive powers the radio forever to save a handshake you pay once.}
...
\begin{frame}[embedded,eyebrow=Lab 1]{Lab 1 setup}
  \vskip 1mm
  \begin{grid}[2]
...
\begin{frame}[embedded,eyebrow=Project]{Project device requirements}
  \vskip 2mm
  \begin{grid}[2]
