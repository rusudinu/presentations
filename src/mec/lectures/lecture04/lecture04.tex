...
\objectivesframe{
  \cell[\faSatelliteDish]{Transport selection}{Why polling a sensor is wrong, what the four push options are, and where MQTT fits.}
  \cell[\faIcon{project-diagram}]{MQTT in practice}{Topics and wildcards, QoS levels, retained messages, last will, keep-alive and sessions.}
  \cell[\faLock]{Device security}{TLS with certificates or pre-shared keys, one identity per device, and credential rotation.}
  \cell[\faIcon{sync-alt}]{Over-the-air updates}{Two partitions, a signature check and a rollback, so a bad image does not brick the board.}
}

% =====================================================================
\divider{Part 1 · Pushing}{Push-based data delivery}[Polling, push mechanisms, and brokers]

\begin{frame}[embedded,eyebrow=Pushing]{Limitations of polling}
  \vskip 2mm
  \begin{columns}[T]
    ...
    \begin{column}{0.40\textwidth}
      \begin{hint}[The cost is the radio]
        On a battery the cost is waking the Wi-Fi radio on every interval, even when nothing changed.
      \end{hint}
    \end{column}
  \end{columns}
  \begin{takeaway}{Publishing on events costs less than polling.}
...
\begin{frame}[eyebrow=Pushing]{Push mechanisms}
  \vskip 2mm
...
\begin{frame}[embedded,eyebrow=Pushing]{The MQTT broker model}
  \vskip 3mm
  \begin{grid}[3]
  ...
  \begin{takeaway}{The device never learns who is listening.}
    You add a consumer by subscribing, not by reflashing the board.
  \end{takeaway}
...
% =====================================================================
\divider{Part 2 · MQTT}{The protocol in detail}[Topics, quality of service, retained state, and sessions]
...
\begin{frame}[embedded,eyebrow=MQTT]{QoS costs and limits}
  \vskip 2mm
  \begin{columns}[T]
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.40\textwidth}
      \begin{hint}[Battery first]
        A session held open exists to receive commands quickly. If the device only publishes, connect, send, disconnect and sleep.
      \end{hint}
    \end{column}
  \end{columns}
...
\begin{frame}[eyebrow=MQTT]{MQTT 5}
  \vskip 2mm
  \begin{grid}[3]
...
% =====================================================================
\divider{Part 3 · Both ends}{From the board to the phone}[Local broker, TinyGo publisher, Flutter subscriber]

\begin{frame}[fragile,eyebrow=Both ends]{Local broker setup}
  \vskip 1mm
\begin{shell}
...
\end{shell}
  \vskip 1mm
  \muted{Subscribe in one terminal, publish in another, and watch the line appear.}
...
\begin{frame}[fragile,embedded,eyebrow=Both ends]{TinyGo publisher}
  \vskip 1mm
  \begin{columns}[T]
...
\end{golang}
    ...
    \begin{column}{0.39\textwidth}
      \lead{Wi-Fi comes from a driver package.}{On the ESP32-C3 and the ESP32-S3 the radio comes from the espradio package. Check your target first.}
      \begin{hint}[Order of failure]
        Network first, broker second, sensor third. Print at every step.
      \end{hint}
    \end{column}
  \end{columns}
  \note{Keep this snippet as a shape rather than a recipe: the package paths move faster than the protocol does. It follows the mqtt example in the espradio repository, with natiu-mqtt as the client, and Lab 2 uses the same shape. What does not move is the sequence: join, dial, connect, publish, and check the error at each stage.}
\end{frame}

\begin{frame}[fragile,eyebrow=Both ends]{Flutter subscriber}
  \vskip 1mm
\begin{dart}
...
\begin{frame}[fragile,eyebrow=Both ends]{Reconnection and backgrounding}
\begin{dart}
...
\begin{frame}[eyebrow=Both ends]{Lab broker versus project broker}
  \vskip 2mm
  ...
  \begin{takeaway}{Develop against the local one, demo against the hosted one.}
    A plain 1883 listener is a lab tool.
  \end{takeaway}
...
% =====================================================================
\divider{Part 4 · CoAP}{Request and response on UDP}

\begin{frame}[embedded,eyebrow=CoAP]{CoAP}
  \vskip 1mm
  \begin{grid}[3]
...
\begin{frame}[eyebrow=CoAP]{CoAP versus MQTT}
  \vskip 1mm
  {\usebeamerfont{small}\hyphenpenalty=10000\exhyphenpenalty=10000 \begin{tabular}{@{}l>{\raggedright\arraybackslash}p{42mm}>{\raggedright\arraybackslash}p{46mm}@{}}
    \toprule
...
% =====================================================================
\divider{Part 5 · Trust}{Identity, encryption, updates}

\begin{frame}[embedded,eyebrow=Trust]{Risks of an open broker}
  \vskip 2mm
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{itemize}
  ...
  \begin{takeaway}{A broker is a command channel, not just a data feed.}
    Treat write access like a shell on the board.
  \end{takeaway}
...
\begin{frame}[embedded,eyebrow=Trust]{TLS on constrained devices}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      \begin{itemize}
        \item TLS 1.3 finishes the handshake in one round trip. The expensive parts are the certificate chain and the buffers held while verifying it.
...
\begin{frame}[embedded,eyebrow=Trust]{Certificates versus pre-shared keys}
  ...
  {\usebeamerfont{small}\hyphenpenalty=10000\exhyphenpenalty=10000 \begin{tabular}{@{}l>{\raggedright\arraybackslash}p{42mm}>{\raggedright\arraybackslash}p{44mm}@{}}
    ...
    \bottomrule
  \end{tabular}}
  \begin{takeaway}{Do not compile one shared secret into the firmware.}
...
\begin{frame}[embedded,eyebrow=Trust]{Per-device identity}
  \vskip 2mm
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      \begin{enumerate}
...
\begin{frame}[fragile,embedded,eyebrow=Trust]{Credential rotation and ACLs}
  \vskip 1mm
  \begin{columns}[T]
...
\end{codeblock}
    ...
    \begin{column}{0.50\textwidth}
      \muted{Give every credential a lifetime you can meet, and build the renewal path before the first board ships.}
      \vskip 2mm
...
\begin{frame}[embedded,eyebrow=Trust]{Over-the-air updates}
  \vskip 1mm
...
\begin{frame}[embedded,eyebrow=Trust]{Rollback and signing}
  ...
  \begin{columns}[T]
    ...
    \end{column}
  \end{columns}
  \begin{takeaway}{An unsigned update path is remote code execution.}
    Whoever can answer the update request owns every board you shipped.
  \end{takeaway}
...
\begin{frame}[fragile,embedded,eyebrow=Trust]{Project topic design}
  \vskip 1mm
\begin{codeblock}
...
\end{codeblock}
  ...
  \begin{takeaway}{Write the topic tree down before you write the firmware.}
    It is the contract between the two halves of your project, and changing it later means reflashing a board and editing an app.
  \end{takeaway}
...
\begin{frame}[eyebrow=Recap]{Summary}
  ...
  \begin{enumerate}
    ...
    \item Identity and updates are part of the design. \muted{One credential per device over TLS, and a signed image in a second partition with a rollback.}
  \end{enumerate}
  \vskip 4mm
