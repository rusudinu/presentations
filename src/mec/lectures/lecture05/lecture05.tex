...
\begin{frame}[embedded,eyebrow=Peripheral]{What tinygo.org/x/bluetooth supports}
  ...
  \end{takeaway}
  \note{The same source builds for a laptop with go build and for a board with tinygo flash. The Xtensa ESP32 modules used earlier in this course are not a straightforward BLE target; the RISC-V parts are. Ask for a show of hands on Mac laptops.}
\end{frame}
