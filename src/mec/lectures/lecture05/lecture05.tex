...
\objectivesframe{
  \cell[\faBluetoothB]{The BLE link}{Advertising, scanning, GAP roles, connection parameters and MTU, and what each costs in energy.}
  \cell[\faDatabase]{GATT data model}{Services, characteristics, descriptors and UUIDs, and when to read, write, notify or indicate.}
  \cell[\faLock]{Bluetooth permissions}{Android 12 and later runtime permissions, the iOS usage string, and the background limits on both.}
  \cell[\faMicrochip]{Central and peripheral}{A Flutter central with flutter\_blue\_plus and a Go peripheral with one notifying characteristic.}
}

% =====================================================================
\divider{Part 1 · The link}{Advertising and connection}[Advertising, scanning, connecting, and power parameters]

\begin{frame}[eyebrow=The link]{Classic Bluetooth and BLE}
  \vskip 1mm
...
\begin{frame}[eyebrow=The link]{BLE for sensors}
  \vskip 2mm
  \begin{grid}[3]
...
\begin{frame}[eyebrow=GAP]{GAP roles}
  \vskip 2mm
  \begin{columns}[T]
  ...
  \begin{takeaway}{The central controls the timing.}
    The peripheral proposes parameters and the central decides.
  \end{takeaway}
...
\begin{frame}[eyebrow=GAP]{Advertising}
  \vskip 3mm
  \begin{grid}[2]
...
\begin{frame}[eyebrow=Connection]{Connection parameters}
  \vskip 2mm
  {\usebeamerfont{small}\begin{tabular}{@{}lp{24mm}p{58mm}@{}}
    ...
    \bottomrule
  \end{tabular}}
  \begin{takeaway}{Use peripheral latency to save power.}
...
\begin{frame}[eyebrow=Connection]{MTU and packet size}
  \vskip 3mm
  \begin{grid}[3]
...
% =====================================================================
\divider{Part 2 · GATT}{Services and characteristics}[Services, characteristics, descriptors, UUIDs, and the four operations]

\begin{frame}[eyebrow=GATT]{GATT structure}
  \begin{columns}[T]
...
\begin{frame}[eyebrow=GATT]{16-bit and 128-bit UUIDs}
  \vskip 3mm
  \begin{grid}[2]
...
\begin{frame}[eyebrow=GATT]{Heart-rate service example}
  \vskip 2mm
...
% =====================================================================
\divider{Part 3 · Security}{Pairing, bonding, and encryption}[Association models, security levels, and private addresses]

\begin{frame}[eyebrow=Security]{Pairing and bonding}
  \vskip 3mm
  \begin{grid}[3]
  ...
  \end{grid}
  \begin{takeaway}{A bonded sensor reconnects silently.}
    An unbonded one asks the user every time.
  \end{takeaway}
...
\begin{frame}[eyebrow=Security]{Addresses, privacy, and encryption}
  \vskip 2mm
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
...
% =====================================================================
\divider{Part 4 · The phone}{Bluetooth on the phone}[Android and iOS permissions, flutter\_blue\_plus]

\begin{frame}[eyebrow=Permissions]{Build-time and runtime permissions}
  \vskip 2mm
  \begin{columns}[T]
...
\begin{frame}[fragile,eyebrow=Permissions]{Android manifest entries}
\begin{codeblock}
...
\begin{frame}[eyebrow=Permissions]{Android Bluetooth permissions by version}
  \vskip 2mm
...
\begin{frame}[fragile,eyebrow=Flutter]{Connection and service discovery}
  \vskip 1mm
\begin{dart}
...
\begin{frame}[fragile,eyebrow=Flutter]{Subscribing to a characteristic}
  \vskip 1mm
\begin{dart}
final c = services
...
\begin{frame}[fragile,eyebrow=Flutter]{Disconnection and reconnection}
  \vskip 1mm
\begin{dart}
...
\end{dart}
  \begin{takeaway}{A reconnect loop is part of the feature.}
    Sensors go out of range, run flat and get switched off.
  \end{takeaway}
...
% =====================================================================
\divider{Part 5 · The peripheral}{BLE peripherals in Go}[tinygo.org/x/bluetooth, lab set-ups, characteristic design]

\begin{frame}[embedded,eyebrow=Peripheral]{tinygo.org/x/bluetooth support}
  \vskip 2mm
...
\begin{frame}[embedded,eyebrow=Lab 3]{Lab 3 set-ups}
  \vskip 2mm
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
...
\begin{frame}[embedded,eyebrow=Design]{Service design}
  \vskip 2mm
  \begin{grid}[3]
    \cell[\faLayerGroup]{Keep the value small}{Fit one reading in twenty bytes. Scaled integers beat a JSON string.}
    ...
    \cell[\faBatteryHalf]{Let the device sleep}{Use a long interval with peripheral latency.}
  \end{grid}
...
\begin{frame}[eyebrow=Recap]{Summary}
  ...
  \begin{enumerate}
    ...
    \item GATT is discovered on every connection. \muted{Rediscover services after every reconnection and never reuse an old characteristic object.}
