...
\subtitle{Motion sensors, location, camera, NFC, and energy cost}
...
\objectivesframe{
  \cell[\faMicrochip]{Phone sensors}{What each sensor in a phone measures, in which unit, and at what rate.}
  \cell[\faCode]{Sensors in Dart}{The sensors\_plus streams, the sampling period, and where the processing belongs.}
  \cell[\faIcon{map-marker-alt}]{Location}{Accuracy modes, the permission flow on both platforms, and the background rules.}
  \cell[\faBatteryHalf]{Energy cost}{The energy cost of each sensor, batching, and when to drop to a platform channel.}
}

% =====================================================================
\divider{Part 1 · Sensors}{Phone sensors}[Measurements, units, and sensor fusion]

\begin{frame}[embedded,eyebrow=Sensors]{Phone sensor overview}
  \begin{grid}[3]
...
\begin{frame}[embedded,eyebrow=Sensors]{Measurements and units}
  \vskip 1mm
...
\begin{frame}[embedded,eyebrow=Sensors]{Sampling rates}
  \vskip 2mm
  \begin{columns}[T]
...
\begin{frame}[embedded,eyebrow=Sensors]{Sensor fusion}
  \vskip 1mm
  \begin{grid}[3]
...
\begin{frame}[embedded,eyebrow=Sensors]{Sensor axes and screen rotation}
  \vskip 2mm
  \begin{columns}[T]
...
% =====================================================================
\divider{Part 2 · Reading sensors}{Sensors in Dart}[sensors\_plus, sampling periods, and a step counter]

\begin{frame}[fragile,eyebrow=Reading sensors]{sensors\_plus streams}
\begin{dart}
...
\begin{frame}[fragile,eyebrow=Reading sensors]{samplingPeriod}
\begin{dart}
...
\begin{frame}[eyebrow=Reading sensors]{Processing samples off the UI isolate}
  \vskip 2mm
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
    ...
    \end{column}
  \end{columns}
  \begin{takeaway}{Process every sample; update the UI at a lower rate.}
  \end{takeaway}
...
\begin{frame}[eyebrow=Step counter]{Step detection with the accelerometer}
  \vskip 2mm
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
    ...
    \end{column}
  \end{columns}
  \begin{takeaway}{Peak counting is enough for a step counter.}
...
\begin{frame}[fragile,eyebrow=Step counter]{Step counter: magnitude and moving baseline}
\begin{dart}
import 'dart:math';
...
\begin{frame}[fragile,eyebrow=Step counter]{Step counter: threshold and debounce}
\begin{dart}
...
\begin{frame}[eyebrow=Step counter]{Step counter tuning and failure cases}
  \vskip 2mm
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
...
% =====================================================================
\divider{Part 3 · Location}{Location}[Accuracy modes, permissions, and background rules]

\begin{frame}[eyebrow=Location]{Position sources}
  \vskip 1mm
  \begin{grid}[3]
  ...
  \begin{takeaway}{Every position carries its own error estimate.}
    \code{Position.accuracy} is a radius in meters.
  \end{takeaway}
...
\begin{frame}[eyebrow=Location]{geolocator accuracy modes and cost}
  \vskip 1mm
...
\begin{frame}[fragile,eyebrow=Location]{Location permission declarations}
...
\end{codeblock}
  ...
  \begin{takeaway}{Write the usage string for the user.}
...
\begin{frame}[fragile,eyebrow=Location]{Permission request flow}
\begin{dart}
...
\begin{frame}[eyebrow=Location]{Location permission levels}
  \vskip 1mm
...
\begin{frame}[eyebrow=Location]{Background location}
  \vskip 2mm
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
...
\begin{frame}[fragile,eyebrow=Location]{Position stream with a distance filter}
\begin{dart}
...
\end{dart}
  \vskip 1mm
  \muted{With a distance filter, a phone on a desk produces no events at all. Use \code{getCurrentPosition} when one fix is enough, and never leave a stream behind a screen the user has left.}
...
% =====================================================================
\divider{Part 4 · Camera and NFC}{Camera and NFC APIs}[Controllers, frames, tags, and iOS restrictions]
...
\begin{frame}[eyebrow=Camera]{takePicture and startImageStream}
  \vskip 2mm
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
...
\begin{frame}[fragile,eyebrow=Camera]{Image stream back-pressure}
\begin{dart}
bool _busy = false;
...
\begin{frame}[eyebrow=NFC]{NFC: tags, NDEF, and platform differences}
  \vskip 2mm
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
...
\begin{frame}[fragile,eyebrow=NFC]{Reading an NFC tag in Dart}
\begin{dart}
final a = await NfcManager.instance.checkAvailability();
if (a != NfcAvailability.enabled) return;  // no reader, or switched off
NfcManager.instance.startSession(
  pollingOptions: {NfcPollingOption.iso14443},
  onDiscovered: (NfcTag tag) async {
    final ndef = Ndef.from(tag);   // from nfc_manager_ndef
    if (ndef == null) return NfcManager.instance.stopSession();
    _show(ndef.cachedMessage);
    await NfcManager.instance.stopSession();
  });
\end{dart}
  \muted{Check availability, start a session with a polling option, inspect the tag, always stop it. The NDEF classes live in a companion package, and signatures have moved across major releases, so read the README.}
...
% =====================================================================
\divider{Part 5 · Energy}{Sensor energy cost}[Cost per sensor, batching, and platform channels]

\begin{frame}[embedded,eyebrow=Energy]{Energy cost per sensor}
  \vskip 1mm
  {\usebeamerfont{small}\begin{tabular}{@{}llp{58mm}@{}}
    ...
    \bottomrule
  \end{tabular}}
  \begin{takeaway}{The sensor itself is rarely the main cost.}
...
\begin{frame}[embedded,eyebrow=Energy]{Sensor batching}
  \vskip 2mm
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
...
\begin{frame}[eyebrow=Energy]{Platform channels for sensors}
  \vskip 1mm
  \begin{grid}[3]
