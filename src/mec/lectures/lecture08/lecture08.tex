...
\subtitle{Offline-first design, local database, sync engine}
...
\objectivesframe{
  \cell[\faCloud]{Offline-first design}{Why offline is a normal state, and why the UI reads local data, never a server response.}
  \cell[\faDatabase]{Local data layer}{A Drift table with sync metadata, and a live query the UI watches with watch().}
  \cell[\faIcon{exchange-alt}]{Sync engine}{An outbox, a drain loop, and a delta pull driven by a checkpoint.}
  \cell[\faSatelliteDish]{Sync triggers}{When to wake the radio, and what background sync can and cannot promise.}
}

\divider{Part 1 · Offline-first}{Offline-first design}

\begin{frame}[eyebrow=Offline-first]{Offline as a normal state}
  \vskip 2mm
  \begin{grid}[3]
  ...
  \end{grid}
  \vskip 3mm
  \muted{Worse than no signal is lie-fi: the interface reports a connection, and the request hangs until the timeout fires.}
...
\begin{frame}[eyebrow=Offline-first]{Common causes of connectivity loss}
  \vskip 2mm
  \begin{grid}[3]
    \cell[\faIcon{building}]{Lifts and basements}{Concrete and steel block a radio signal completely, for the whole ride.}
    \cell[\faIcon{plane}]{Planes and trains}{A tunnel repeats the same loss every few minutes.}
    \cell[\faWifi]{Hotel Wi-Fi}{DNS resolves, the interface reports a connection, and nothing routes past it.}
  \end{grid}
  \vskip 4mm
  \muted{Hospital wards, rural roads, and festival crowds add the same failure by radio congestion, not by radio absence.}
...
\begin{frame}[eyebrow=Offline-first]{Online-first versus offline-first}
...
\begin{frame}[eyebrow=Offline-first]{CAP theorem on a mobile device}
  \vskip 1mm
  ...
  \vskip 3mm
  \begin{grid}[3]
    \cell[\faWifi]{Partition tolerance}{Not a choice on mobile.}
    ...
    \cell[\faIcon{balance-scale}]{Consistency}{You give it up. What you get instead is eventual consistency.}
  \end{grid}
...
\begin{frame}[eyebrow=Offline-first]{Offline support in the project}
  \vskip 2mm
  \begin{grid}[2]
    \cell[\faIcon{list-ol}]{Requirement 3 of 5}{The app must work with no network and sync later, with a stated conflict rule.}
    \cell[\faClock]{Milestone 7}{By the seventh project session the app runs offline and syncs. Lab 5 builds this.}
  \end{grid}
...
\divider{Part 2 · Local database}{Local database as the source of truth}[Schema, reactive reads, outbox]

\begin{frame}[eyebrow=Local database]{Firestore offline cache versus Drift}
...
\begin{frame}[eyebrow=Local database]{Sync columns}
  \vskip 2mm
  \begin{grid}[2]
...
\begin{frame}[fragile,eyebrow=Local database]{Reactive reads from the local database}
  \vskip 1mm
\begin{dart}
...
\begin{frame}[fragile,eyebrow=Local database]{Writing to the table and the outbox}
  \vskip 1mm
\begin{dart}
...
\begin{frame}[fragile,eyebrow=Local database]{Deletes as tombstones}
  \vskip 1mm
\begin{dart}
...
\begin{frame}[eyebrow=Local database]{Outbox versus direct API calls}
  \vskip 2mm
  \begin{grid}[3]
...
\divider{Part 3 · Sync engine}{Sync engine}[Push loop, pull loop, checkpoint]

\begin{frame}[eyebrow=Sync engine]{Push and pull loops}
  ...
  \begin{columns}[T]
    ...
    \end{column}
  \end{columns}
  \begin{takeaway}{The checkpoint is the server's timestamp.}
    Never the one your device read, or clock skew makes you skip rows.
  \end{takeaway}
...
\begin{frame}[fragile,eyebrow=Sync engine]{FIFO ordering and failure handling}
  \vskip 1mm
\begin{dart}
try {
  await api.push(item, idempotencyKey: item.id);
...
\begin{frame}[eyebrow=Sync engine]{Idempotency keys}
  \vskip 2mm
...
\begin{frame}[fragile,eyebrow=Sync engine]{Delta sync}
...
\begin{frame}[fragile,eyebrow=Sync engine]{Sync response format}
\begin{codeblock}
...
\begin{frame}[eyebrow=Sync engine]{Sync parameters}
  \vskip 4mm
  \begin{grid}[3]
...
\begin{frame}[eyebrow=Sync engine]{Push, polling, and triggered sync}
...
\begin{frame}[fragile,eyebrow=Sync engine]{Background sync}
  \begin{columns}[T]
...
\end{dart}
    ...
    \end{column}
  \end{columns}
  \begin{takeaway}{Only foreground triggers are reliable.}
  \end{takeaway}
...
\begin{frame}[eyebrow=Sync engine]{Lab 5 requirements}
  \vskip 2mm
  \begin{grid}[2]
...
\begin{frame}[eyebrow=Recap]{Summary}
  ...
  \begin{enumerate}
    ...
    \item Sync on events, not on a timer. \muted{App resumed, connectivity returned, or the user asked.}
  \end{enumerate}
  \vskip 5mm
