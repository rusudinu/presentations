...
\subtitle{Logical clocks, conflict resolution, CRDTs}
...
\objectivesframe{
  \cell[\faClock]{Event ordering}{Why a wall clock cannot order two edits, and what a Lamport or vector clock gives you instead.}
  \cell[\faIcon{code-branch}]{Resolution strategies}{Last writer wins, first writer wins, merge by field, ask the user, and what each one costs.}
  \cell[\faCode]{Conflict resolver}{One Dart function that both devices run and that both devices agree on.}
  \cell[\faLayerGroup]{CRDTs}{Counters, registers and sets whose merge is commutative, associative and idempotent.}
}

% =====================================================================
\divider{Part 1 · Conflicts}{Sync conflicts}[Sources of conflicts, device clocks]

\begin{frame}[eyebrow=Conflicts]{Recap of Lecture 8}
  \vskip 2mm
  \begin{grid}[3]
...
\begin{frame}[eyebrow=Conflicts]{Sources of conflicts}
  \vskip 3mm
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      \lead{Update and update}{Both devices set the title of the same note.}
      \vskip 2mm
...
\begin{frame}[eyebrow=Conflicts]{Clock skew between devices}
  \vskip 2mm
...
\begin{frame}[eyebrow=Conflicts]{Happens-before and concurrency}
  \vskip 2mm
  \begin{grid}[3]
...
% =====================================================================
\divider{Part 2 · Ordering}{Logical clocks}[Lamport clocks, vector clocks, hybrid logical clocks]

\begin{frame}[fragile,eyebrow=Ordering]{Lamport clocks}
  \begin{columns}[T]
...
\end{dart}
    ...
    \end{column}
  \end{columns}
  \begin{takeaway}{The converse is false.}
    It does not prove causality: the two events may have been concurrent.
  \end{takeaway}
...
\begin{frame}[fragile,eyebrow=Ordering]{Vector clocks}
\begin{dart}
...
\begin{frame}[eyebrow=Ordering]{Worked example: vector clocks on one note}
  \vskip 2mm
  {\usebeamerfont{small}
  ...
  \end{tabular}}
  \vskip 3mm
  \muted{P is the phone, L the laptop. In step 5, L does not guess: its vector is smaller in every slot.}
...
\begin{frame}[fragile,eyebrow=Ordering]{Hybrid logical clocks}
  \begin{columns}[T]
...
% =====================================================================
\divider{Part 3 · Strategies}{Conflict resolution strategies}

\begin{frame}[eyebrow=Strategies]{Resolution strategies compared}
  \vskip 1mm
  {\usebeamerfont{small}
...
\end{dart}
      \vskip 1mm
      \muted{One comparison, no per-entity code.}
    \end{column}
    \begin{column}{0.46\textwidth}
...
\begin{frame}[eyebrow=Strategies]{First writer wins}
  ...
  \begin{takeaway}{First writer wins needs a natural key.}
    Without one the two creates are two different rows, and the rule never fires. A unique constraint on the server enforces it, not a timestamp.
  \end{takeaway}
...
\begin{frame}[fragile,eyebrow=Strategies]{Resolver implementation}
\begin{dart}
...
% =====================================================================
\divider{Part 4 · CRDTs}{Conflict-free replicated data types}

\begin{frame}[eyebrow=CRDTs]{CRDT merge properties}
  \vskip 2mm
...
\begin{frame}[eyebrow=CRDTs]{Join-semilattices}
  \vskip 3mm
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      ...
      \vskip 2mm
      \lead{Merge is the least upper bound.}{$x \sqcup y$ is the smallest state containing both: nothing invented, nothing dropped. Concurrent states are incomparable.}
    \end{column}
    \begin{column}{0.46\textwidth}
...
\begin{frame}[eyebrow=CRDTs]{Grow-only counter}
  \vskip 2mm
...
\end{dart}
  \begin{takeaway}{The value is derived from the map.}
...
\begin{frame}[fragile,eyebrow=CRDTs]{PN-Counter and LWW-Register}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
...
\begin{frame}[eyebrow=CRDTs]{Set CRDTs: G-Set and 2P-Set}
  \vskip 2mm
  \begin{grid}[3]
...
\begin{frame}[eyebrow=CRDTs]{The observed-remove set}
  ...
  \begin{takeaway}{Add wins, because the remover could not have observed it.}
    A defensible default for a shopping list and the wrong one for a bank ledger.
  \end{takeaway}
...
% =====================================================================
\divider{Part 5 · Costs}{Tombstones, growth, and limits}[Storage overhead, alternatives to CRDTs]

\begin{frame}[eyebrow=Costs]{Tombstones}
  \vskip 3mm
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      \begin{itemize}
...
\begin{frame}[eyebrow=Costs]{CRDT storage overhead}
  \vskip 3mm
  \begin{grid}[3]
  ...
  \begin{takeaway}{A CRDT stores enough history to make every merge deterministic.}
  \end{takeaway}
...
\begin{frame}[eyebrow=Costs]{Limits of CRDTs}
  \vskip 2mm
  \begin{grid}[3]
...
\begin{frame}[eyebrow=Costs]{Choosing a strategy for the project}
  {\usebeamerfont{small}
  \begin{tabular}{@{}lll@{}}
    \toprule
  ...
  \end{tabular}}
  \vskip 2mm
  \begin{takeaway}{The project asks for a stated conflict rule.}
