...
\subtitle{Serverless, VPS hosting, Go backends, API design}
...
\objectivesframe{
  \cell[\faIcon{code-branch}]{Client and server responsibilities}{What must run on a machine you control, and why the client is untrusted input.}
  \cell[\faCloud]{Serverless versus a VPS}{Cold starts, scaling and cost shape, and the case where each one wins.}
  \cell[\faIcon{plug}]{API design for mobile}{Pagination, versioning, conditional requests, compression and rate limits.}
  \cell[\faServer]{Go backend deployment}{A minimal Docker image, a systemd unit, environment variables, health checks and logs.}
}

\divider{Part 1 \textperiodcentered\ Server and client}{Client versus server placement}
  ...
  \begin{columns}[T]
    \begin{column}{0.6\textwidth}
      \begin{itemize}
        ...
        \item One request and one response cross the network between them. On mobile that round trip costs roughly 40–200 ms on a good connection, seconds on a bad one.
      \end{itemize}
    \end{column}
...
\begin{frame}[eyebrow=Server and client]{Untrusted client input}
  \begin{columns}[T]
...
\divider{Part 2 \textperiodcentered\ Where the server runs}{Serverless versus a VPS}[Cost, failure modes, operational work]
...
\begin{frame}[eyebrow=Serverless]{Cold starts: the latency of the first request}
  ...
  \end{itemize}
  \muted{Minimum instances, or provisioned concurrency, mean paying for idle capacity, the cost serverless was meant to remove. A smaller package and a fast-starting runtime cost nothing extra.}
...
\begin{frame}[eyebrow=VPS]{Virtual private servers}
  \begin{columns}[T]
    \begin{column}{0.6\textwidth}
      \begin{itemize}
...
\begin{frame}[eyebrow=Serverless vs VPS]{Serverless and VPS cost comparison}
...
\begin{frame}[eyebrow=Serverless]{Writing code for a FaaS runtime}
  \begin{grid}[3]
...
\begin{frame}[eyebrow=VPS]{Operating a VPS}
  \vskip 2mm
  \begin{grid}[3]
    ...
    \cell[\faIcon{sync-alt}]{Run it as a service}{systemd or Docker with a restart policy.}
...
\divider{Part 3 \textperiodcentered\ Go backends}{Go backends}[Static binaries, fast startup, cross-compilation]

\begin{frame}[fragile,eyebrow=Go backends]{Go for small mobile backends}
  \begin{columns}[T]
    \begin{column}{0.6\textwidth}
...
\begin{frame}[fragile,eyebrow=Go backends]{Go handler and Dart client call}
  \begin{columns}[T]
  ...
  \begin{columns}[T]
    \begin{column}{0.62\textwidth}
      \begin{itemize}
        ...
        \item Hedging is the opposite problem: a small fraction of requests are slow. Send a second copy after a short delay, take whichever answers first, and cancel the other.
      \end{itemize}
    \end{column}
...
\divider{Part 4 \textperiodcentered\ API design for a mobile client}{API design for mobile clients}[Pagination, versioning, conditional requests, compression, and rate limits]

\begin{frame}[fragile,eyebrow=API design]{Cursor-based pagination}
  \begin{columns}[T]
...
\begin{frame}[eyebrow=API design]{API versioning}
  \vskip 1mm
  \begin{itemize}
...
\begin{frame}[eyebrow=API design]{Response compression}
  \vskip 1mm
  \begin{itemize}
...
\begin{frame}[eyebrow=API design]{Rate limiting}
  \vskip 1mm
  \begin{itemize}
    ...
    \item The server answers 429 Too Many Requests with a \code{Retry-After} header, so a well-behaved client waits that long instead of guessing.
  ...
  \end{itemize}
  \begin{takeaway}{Expose the rate limit in response headers.}
  \end{takeaway}
...
\divider{Part 5 \textperiodcentered\ Deploying and operating}{Deployment and operations}[Docker, Cloud Run, systemd, secrets, health checks, logs]
...
\begin{frame}[fragile,eyebrow=Deploying]{Deploying to Cloud Run}
  \begin{itemize}
...
\end{codeblock}
    ...
    \begin{column}{0.4\textwidth}
      ...
      \muted{\code{Restart=always} with a short \code{RestartSec} means a crash recovers in about two seconds.}
    \end{column}
  \end{columns}
...
\begin{frame}[fragile,eyebrow=Deploying]{Structured logging}
  \begin{columns}[T]
    \begin{column}{0.58\textwidth}
      \begin{itemize}
...
\begin{frame}[eyebrow=Deploying]{Project backend requirements}
  \vskip 1mm
  \begin{itemize}
    ...
    \item Keep it on one small VPS or one Cloud Run service for the whole semester.
    \item Everything in this lecture applies at project scale: environment variables for broker and database URLs, a health check the grading script can poll, and structured logs.
  \end{itemize}
...
\begin{frame}[eyebrow=Recap]{Summary}
  ...
  \begin{enumerate}
    ...
    \item Serverless bills per request and cold-starts; a VPS bills per month and never does. \muted{Pick by cost shape.}
