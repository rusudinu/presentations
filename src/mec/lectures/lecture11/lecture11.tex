...
\subtitle{Protobuf contracts, streaming, wire format, transport}
...
\objectivesframe{
  \cell[\faServer]{Service contracts}{Write a .proto file and generate Go and Dart stubs with protoc, from one source of truth.}
  \cell[\faCode]{Call shapes}{Unary, server streaming, client streaming and bidirectional, and know which one a screen needs.}
  \cell[\faBolt]{Wire format}{Why protobuf is smaller than JSON, how a schema evolves safely, and what HTTP/2 buys for free.}
  \cell[\faClock]{Deadlines and cancellation}{Deadlines, cancellation and the radio tail on a mobile client.}
}

\begin{frame}[eyebrow=Overview]{gRPC overview}
  \vskip 3mm
  \begin{grid}[4]
...
\divider{Part 1 · Contracts}{Contract-first design}[RPC versus REST, .proto contracts]

\begin{frame}[fragile,eyebrow=Contracts]{RPC versus REST}
  \begin{columns}[T]
    \begin{column}{0.53\textwidth}
      \begin{itemize}
        ...
        \item \textbf{On a phone the difference is round trips.} \muted{A resource-shaped screen often needs three or four sequential requests, and each one costs time on cellular before any data moves.}
      \end{itemize}
    \end{column}
...
\begin{frame}[eyebrow=Contracts]{Choosing between REST and gRPC}
  ...
  \end{center}
  \vskip 2mm
  \begin{takeaway}{gRPC fits where you own both ends of the call; REST fits where you do not.}
  \end{takeaway}
...
\begin{frame}[fragile,eyebrow=Contracts]{The .proto contract}
  \begin{columns}[T]
...
\end{codeblock}
    ...
    \end{column}
  \end{columns}
  \note{Write this live if there is time. The thing students miss: proto3 enums must have a zero value meaning unspecified, because an absent enum always decodes as 0. Versioning the package, not the file, is worth saying out loud too. Mention Protobuf Editions: a new file can open with \texttt{edition = ``2024'';} instead of a syntax line, which replaces the proto2 and proto3 designations without changing the wire format.}
\end{frame}
...
\begin{frame}[eyebrow=Contracts]{Generated code}
  \vskip 2mm
  \begin{itemize}
...
\divider{Part 2 · Implementing}{A Go server and a generated Dart client}[Unary calls, server streaming, client streaming]
...
\begin{frame}[eyebrow=Implementing]{gRPC status codes}
  \vskip 1mm
  \begin{center}
  {\usebeamerfont{small}
  ...
  \end{center}
  \vskip 2mm
  \begin{takeaway}{The client branches on \code{e.code}, not on a parsed string.}
  \end{takeaway}
...
\end{dart}
    ...
    \begin{column}{0.37\textwidth}
      {\usebeamerfont{small}
      \begin{itemize}
        \item \textbf{It looks like a local call.} \muted{It is still a network call.}
...
\begin{frame}[fragile,eyebrow=Implementing]{Client streaming: draining an offline queue}
  \begin{columns}[T]
    \begin{column}{0.47\textwidth}
      Many requests, one response: the shape of an outbox drain from Lecture 8. Queue rows offline, then stream them upstream for one acknowledgement.
    \end{column}
...
\end{dart}
    \end{column}
  \end{columns}
  \note{Client streaming is the shape offline sync from Lecture 8 actually needs: drain the outbox in order and get one acknowledgement, not one round trip per row.}
\end{frame}

\divider{Part 3 · The wire}{Wire format and schema evolution}[Protobuf encoding size, schema evolution]

\begin{frame}[fragile,eyebrow=Wire format]{Protobuf encoding size}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      {\usebeamerfont{small}
      \begin{itemize}
...
\end{codeblock}
    \end{column}
  \end{columns}
  \begin{takeaway}{The size ratio depends on your schema and data.}
    Protobuf is smaller, often by a large factor. Measure yours.
  \end{takeaway}
...
\begin{frame}[fragile,eyebrow=Wire format]{Schema evolution}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
\begin{codeblock}
message Reading {
...
\end{codeblock}
    ...
    \end{column}
  \end{columns}
  \begin{takeaway}{The wire format is the compatibility contract.}
    A year-old build is still on the network.
  \end{takeaway}
...
\begin{frame}[eyebrow=Call shapes]{Four call shapes}
...
\divider{Part 4 · Transport}{HTTP/2 and HTTP/3 transport}[Multiplexing, QUIC, network changes]

\begin{frame}[eyebrow=Transport]{HTTP/2 multiplexing}
  {\usebeamerfont{small}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{itemize}
    ...
    \end{column}
  \end{columns}}
  \begin{takeaway}{One warm, reused connection is most of gRPC's mobile advantage.}
  \end{takeaway}
...
\begin{frame}[eyebrow=Transport]{HTTP/3 and QUIC on mobile}
  {\usebeamerfont{small}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{itemize}
...
\begin{frame}[eyebrow=Numbers]{Key numbers}
  \vskip 4mm
  \begin{grid}[3]
...
\divider{Part 5 · Reaching everywhere}{Browser support and the project}[gRPC-Web, Connect, Lab 6]

\begin{frame}[fragile,eyebrow=gRPC]{gRPC-Web and Connect}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      {\usebeamerfont{small}
      \begin{itemize}
...
\end{golang}
    ...
    \end{column}
  \end{columns}
  \note{If a team's project ships a Flutter web build, this is the slide that tells them why a raw gRPC channel will not compile for that target. Connect ships Go, TypeScript and Swift clients, and a Dart one is only on the roadmap, so a Flutter web client still talks gRPC-Web.}
\end{frame}

\begin{frame}[eyebrow=Project]{gRPC in Lab 6 and the project}
  \vskip 3mm
  \begin{grid}[2]
  ...
  \end{grid}
  \begin{takeaway}{The exact commands and acceptance criteria are in the Lab 6 handout.}
  \end{takeaway}
...
\begin{frame}[eyebrow=Recap]{Summary}
  \vskip 2mm
  \begin{enumerate}
    \item A .proto is a contract. \muted{Field numbers are the wire format; never renumber or reuse one.}
    ...
    \item Deadlines and cancellation matter on a phone. \muted{Set one, cancel in dispose, and mind the radio tail.}
  \end{enumerate}
  \vskip 5mm
