...
\subtitle{Platform channels, dart:ffi, Kotlin Multiplatform}
...
\objectivesframe{
  \cell[\faIcon{exchange-alt}]{Platform channels}{MethodChannel and EventChannel: calling into native code and receiving a native stream.}
  \cell[\faBolt]{dart:ffi}{dart:ffi and ffigen: calling C directly, and generating the bindings instead of hand-writing them.}
  \cell[\faLayerGroup]{Kotlin Multiplatform}{Kotlin Multiplatform: one module, expect/actual, consumed natively by Android and iOS.}
  \cell[\faIcon{balance-scale}]{Choosing a boundary}{Channels for control, FFI for compute, and what a compile-time boundary buys over a run-time one.}
}

\begin{frame}[eyebrow=Framing]{Native interop options}
  \vskip 2mm
  \begin{grid}[3]
    \cell[\faIcon{envelope}]{A message}{A platform channel encodes a call, hands it to the other side, and decodes the reply.}
...
% =====================================================================
\divider{Part 1 · Channels}{Platform channels}[MethodChannel, EventChannel, call cost]

\begin{frame}[eyebrow=Channels]{Dart and platform runtimes}
  \vskip 1mm
  \begin{grid}[2]
  ...
  \begin{takeaway}{Every crossing is one of two things.}
...
\begin{frame}[eyebrow=Channels]{Three platform channel pitfalls}
  ...
  \begin{grid}[3]
    ...
    \cell[\faIcon{hourglass-half}]{The handler is on the main thread}{Do the work on another thread and call \code{result} later, or the native UI itself will freeze.}
  ...
  \end{takeaway}
  \note{These three are the bugs students actually hit in the lab: editing Kotlin and expecting hot reload to pick it up, freezing the native UI from inside a handler, and two packages both claiming the same channel name. Mention pigeon, the Flutter team's generator: one Dart file describes the interface and it emits the Dart, Kotlin and Swift sides, so the method names and argument types stop being strings.}
\end{frame}
...
\begin{frame}[eyebrow=Channels]{Channel call cost}
  \begin{columns}[T]
    \begin{column}{0.58\textwidth}
...
\begin{frame}[fragile,eyebrow=Channels]{Measuring channel and FFI cost}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
\begin{dart}[aboveskip=1mm,belowskip=1mm]
...
\end{dart}
    ...
    \end{column}
  \end{columns}
  \begin{takeaway}{A channel call runs microseconds to low milliseconds. An FFI call stays flat.}
  \end{takeaway}
...
% =====================================================================
\divider{Part 2 · dart:ffi}{dart:ffi}[Calling C, generating bindings with ffigen]
...
\end{dart}
    ...
    \end{column}
  \end{columns}
  \note{The struct declaration mirrors the C layout field for field. On Android, dart:ffi reaches C, not Kotlin: calling into Kotlin from here still means going through JNI, which is its own cost, and jnigen is the tool that generates those bindings. On iOS the C symbols are already in the process, so DynamicLibrary.process finds them directly.}
\end{frame}

\begin{frame}[fragile,eyebrow=FFI]{ffigen: the C header}
...
\end{codeblock}
  \begin{takeaway}{Do not hand-write the struct and the lookup.}
    ffigen reads this header and generates the Dart \code{Struct}, the typedefs and the lookup calls for every function in it.
  \end{takeaway}
...
\end{dart}
    \end{column}
  \end{columns}
  \note{One command regenerates the whole file. Point out that the generated class name, SigBindings, is exactly what was set in the ffigen config, and that this file should be treated as build output: never hand-edit it. A build hook, hook/build.dart, is the current way to compile and bundle the C library with the package, so the app does not carry a prebuilt .so by hand.}
\end{frame}
...
\begin{frame}[eyebrow=FFI]{Zero-copy large buffers}
...
\begin{frame}[eyebrow=FFI]{Platform channels versus dart:ffi}
...
% =====================================================================
\divider{Part 3 · Kotlin Multiplatform}{Compile-time code sharing}[expect/actual, source sets, iOS integration]

\begin{frame}[eyebrow=KMP]{Shared logic, native UI}
...
\begin{frame}[fragile,eyebrow=KMP]{expect and actual: common declaration}
  \begin{columns}[T]
...
\begin{frame}[fragile,eyebrow=KMP]{expect and actual: platform implementations}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
...
\begin{frame}[fragile,eyebrow=KMP]{Source sets}
  \begin{columns}[T]
...
\begin{frame}[fragile,eyebrow=KMP]{iOS integration}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
\begin{shell}[aboveskip=1mm,belowskip=1mm]
...
\begin{frame}[eyebrow=KMP]{Flutter and Kotlin Multiplatform}
  {\usebeamerfont{small}\renewcommand{\arraystretch}{1.05}\begin{tabular}{@{}lp{50mm}p{50mm}@{}}
    ...
    \bottomrule
  \end{tabular}}
  \begin{takeaway}{Flutter crosses the boundary at run time. Kotlin Multiplatform crosses it at compile time.}
  \end{takeaway}
...
\begin{frame}[eyebrow=Project]{Native interop in the project}
  \vskip 2mm
