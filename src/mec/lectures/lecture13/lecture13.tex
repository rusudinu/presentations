...
\subtitle{Frame budget, widget trees, optimization, profiling}
...
\objectivesframe{
  \cell[\faIcon{tachometer-alt}]{Frame budget}{One frame is 1 \(\div\) refresh rate. See where Impeller and the raster thread spend it.}
  \cell[\faLayerGroup]{Widget, Element and RenderObject trees}{Widget, Element and RenderObject, and how reconciliation decides what a rebuild touches.}
  \cell[\faBolt]{Optimization techniques}{const constructors, builder lists, and when an offscreen buffer or a decoded image is the problem.}
  \cell[\faIcon{search}]{Profiling}{Read the DevTools frame chart, tell a UI-thread problem from a raster-thread one, and fix a real list screen.}
}

\divider{Part 1 \textperiodcentered\ Performance}{Speed and efficiency}[Startup latency, frame budget]

\begin{frame}[eyebrow=Performance]{Speed versus efficiency}
  \begin{columns}[T]
...
\begin{frame}[eyebrow=Performance]{Startup latency: cold, warm, hot}
  ...
  {\usebeamerfont{small}\begin{tabular}{@{}p{18mm}p{78mm}p{28mm}@{}}
    ...
    \bottomrule
  \end{tabular}}
  \begin{takeaway}{Measure the one your users hit.}
...
\begin{frame}[eyebrow=Performance]{One frame budget \(=\) 1 \(\div\) refresh rate}
  ...
  \begin{grid}[3]
    ...
    \bignum{11.1 ms}{90 Hz: many mid-range and older phones}
...
\divider{Part 2 \textperiodcentered\ Rendering}{Flutter rendering}[Three trees, reconciliation, operation costs]

\begin{frame}[eyebrow=Rendering]{Flutter rendering and Impeller}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      \begin{itemize}
        \item Flutter does not draw the platform's native widgets. It ships a renderer and paints the entire surface itself.
        \item The renderer is \textbf{Impeller}, not Skia. iOS runs Impeller only. Android enables it by default on API 29 and later, and falls back to the legacy OpenGL renderer below that. Flutter on the web still draws with Skia.
      \end{itemize}
    \end{column}
    \begin{column}{0.40\textwidth}
      \begin{hint}[Why it exists]
        Skia compiled a shader the first time an effect appeared: a stutter on first use. Impeller precompiles its shaders when the engine is built.
      \end{hint}
    \end{column}
  \end{columns}
...
\begin{frame}[eyebrow=Rendering]{Shader warm-up under Impeller}
  \begin{itemize}
    ...
    \item Impeller precompiles its shader set when the engine itself is built. The first frame that uses an effect costs the same as the thousandth, so there is no first-run jank left to warm up.
  \end{itemize}
...
\begin{frame}[eyebrow=Rendering]{The three trees and their costs}
  \muted{ADMD Lectures 3 and 5 introduced the three trees at the developer level. Here is what each one costs to touch.}
  \vskip 3mm
  \begin{grid}[3]
    \cell[\faIcon{clone}]{Widget tree}{The blueprint. Immutable, no behavior. Allocating one is a bump-pointer write: constant time.}
...
\begin{frame}[eyebrow=Rendering]{StatelessWidget rebuilds}
  \begin{itemize}
  ...
  {\usebeamerfont{small}\begin{tabular}{@{}p{28mm}p{46mm}p{46mm}@{}}
    ...
    \bottomrule
  \end{tabular}}
  \begin{takeaway}{Something above a StatelessWidget decides when it rebuilds.}
  \end{takeaway}
...
\begin{frame}[fragile,eyebrow=Rendering]{Reconciliation: runtimeType and key}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
\begin{dart}
...
\end{dart}
    ...
    \begin{column}{0.40\textwidth}
      \begin{itemize}
        ...
        \item Keeping types and keys stable keeps the answer "true".
      \end{itemize}
    \end{column}
  \end{columns}
...
\begin{frame}[eyebrow=Rendering]{setState and dirty elements}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      \begin{itemize}
...
\begin{frame}[fragile,eyebrow=Rendering]{Keys and element identity}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \begin{itemize}
...
\begin{frame}[eyebrow=Rendering]{GlobalKey and reparenting}
  \begin{columns}[T]
  ...
  \begin{takeaway}{Keys are cheap. GlobalKeys are not.}
    Use a GlobalKey when you need to reparent or reach a State from outside it, not as a convenient handle to a widget.
  \end{takeaway}
...
\begin{frame}[eyebrow=Rendering]{Cost of tree operations}
  \vskip 1mm
  ...
  \begin{takeaway}{Layout is one depth-first pass: constraints go down, sizes come up.}
    A relayout boundary stops a dirty flag from propagating upward. A fixed-size box around a busy widget is an optimization.
  \end{takeaway}
...
\divider{Part 3 \textperiodcentered\ Optimization}{Optimization techniques}[const, builder lists, offscreen buffers, and images]

\begin{frame}[fragile,eyebrow=Optimization]{const constructors}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
\begin{dart}
...
\end{dart}
    ...
    \end{column}
  \end{columns}
  \begin{takeaway}{Use const constructors wherever the arguments allow it.}
...
\begin{frame}[fragile,eyebrow=Optimization]{Lazy lists with ListView.builder}
...
\end{dart}
    ...
    \end{column}
  \end{columns}
  \begin{takeaway}{The eager version's cost grows with the number of rows.}
  \end{takeaway}
...
\begin{frame}[eyebrow=Optimization]{Offscreen buffers and saveLayer}
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
...
\begin{frame}[eyebrow=Optimization]{RepaintBoundary}
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      \begin{itemize}
  ...
  \begin{takeaway}{A layer is a deliberate trade: less repaint spread, for more video memory.}
    Reach for it once you have profiled a repaint problem, not by default.
  \end{takeaway}
...
\begin{frame}[eyebrow=Optimization]{Image decoding and memory}
  \begin{itemize}
    ...
    \item A screen with many such images, a photo grid or a long avatar list, can allocate far more memory than the screen shows, and force needless garbage collection.
  \end{itemize}
...
\divider{Part 4 \textperiodcentered\ Profiling}{Profiling}[DevTools performance view, before-and-after example]

\begin{frame}[fragile,eyebrow=Profiling]{Profile mode builds}
\begin{shell}
...
\begin{frame}[eyebrow=Profiling]{The DevTools frame chart}
  \begin{itemize}
    ...
    \item Click a bar to expand it into a flame chart of the calls that ran during that frame: build, layout, paint, and everything beneath them.
  \end{itemize}
  \begin{takeaway}{A tall, wide band across many frames is a systemic problem.}
    One isolated tall bar is usually a single expensive event, like a navigation.
  \end{takeaway}
...
\begin{frame}[eyebrow=Profiling]{UI thread versus raster thread}
  \vskip 1mm
  {\usebeamerfont{small}\begin{tabular}{@{}p{44mm}p{80mm}@{}}
    \toprule
    \thead{Signal} & \thead{What it means} \\
    \midrule
    Tall UI segment (light blue) & your Dart is slow: an expensive build(), or isolate-bound work \\
    Tall raster segment (dark blue) & the GPU is slow: a saveLayer, an oversized image, too many layers \\
    ...
    \bottomrule
  \end{tabular}}
  \begin{takeaway}{The taller half tells you which side of the problem you have.}
    Fix a UI segment with less work in build(); fix a raster segment with fewer or smaller layers.
  \end{takeaway}
  \note{Some Flutter documentation still calls the raster thread the "GPU thread"; both names refer to the same segment. Janky frames also carry a red overlay in the chart, which is the fastest thing to scan for.}
\end{frame}

\begin{frame}[fragile,eyebrow=Profiling]{Example: a list screen that drops frames}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
\begin{dart}
...
\end{dart}
    ...
    \end{column}
  \end{columns}
  \begin{takeaway}{The frame chart: a steady band of tall UI segments, plus raster spikes on scroll.}
...
\begin{frame}[fragile,eyebrow=Profiling]{Example: the fix and its effect}
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
\begin{dart}
ListView.builder(
...
\end{dart}
  ...
  \begin{takeaway}{Each fix maps to an earlier slide: builder lists, keys, decoding.}
    The UI band drops under budget, and the raster spikes disappear.
  \end{takeaway}
...
\begin{frame}[eyebrow=Recap]{Summary}
  ...
  \begin{enumerate}
    ...
    \item Profile before you fix. \muted{The taller half of the frame bar tells you whether the problem is your Dart, the UI segment, or the GPU, the raster segment.}
  \end{enumerate}
  \vskip 5mm
