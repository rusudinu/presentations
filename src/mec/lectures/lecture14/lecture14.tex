...
\begin{frame}[eyebrow=Compilation]{Where cross-platform runtimes stand in 2026}
  ...
  \begin{columns}[T]
    \begin{column}{0.47\textwidth}
      ...
      \vskip 2mm
      \muted{Impeller precompiles its shaders when the engine is built, so no shader is compiled on the user's device. Debug keeps the JIT, which is exactly what hot reload is.}
    \end{column}
    \begin{column}{0.47\textwidth}
