...
\subtitle{Compilation, energy, exam preparation, project defense}
...
\objectivesframe{
  \cell[\faCode]{Compilation strategies}{Interpretation, just-in-time and ahead-of-time compilation, and what each one trades away.}
  \cell[\faToggleOn]{Build modes}{Why Dart ships two compilers, what Android's runtime does, and why iOS allows no choice.}
  \cell[\faBolt]{Energy use}{Race to idle, thermal throttling, and the duty cycle that decides a device's battery life.}
  \cell[\faGraduationCap]{Exam preparation}{The exam format, what to revise per lecture, the project defense, and the concept presentation.}
}

% =====================================================================
\divider{Part 1 · Compilation}{Compilation strategies}[Interpretation, JIT, AOT, mobile runtimes]

\begin{frame}[eyebrow=Compilation]{Interpretation, JIT and AOT}
  \vskip 2mm
  {\usebeamerfont{small}
...
\begin{frame}[fragile,eyebrow=Compilation]{Dart build modes}
  \vskip 2mm
  \begin{columns}[T]
...
\end{shell}
    ...
    \begin{column}{0.41\textwidth}
      \lead{JIT or AOT}{"Dart is a JIT language" and "Dart is an AOT language" are both half true. It is one language with two compilers, and the build mode picks one.}
      \vskip 2mm
  ...
  \begin{takeaway}{JIT for the developer loop, AOT for the user.}
    Profile mode is the compromise: release code plus the hooks DevTools needs.
  \end{takeaway}
...
\begin{frame}[eyebrow=Compilation]{JIT advantages over AOT}
  \vskip 2mm
  \begin{columns}[T]
...
\begin{frame}[eyebrow=Compilation]{Android runtime evolution}
  \vskip 2mm
  {\usebeamerfont{small}
  \begin{tabular}{@{}lp{24mm}p{78mm}@{}}
    ...
    Baseline profiles  & Android 7 and later & hot paths compiled from a profile, at install time since Android 9 \\
    \bottomrule
  \end{tabular}}
...
\begin{frame}[eyebrow=Compilation]{iOS and write XOR execute}
  \vskip 2mm
  \begin{columns}[T]
    ...
    \begin{column}{0.44\textwidth}
      ...
      \vskip 2mm
      \muted{WebKit's JavaScript engine is the privileged exception. Bitcode is not a workaround: Apple deprecated it in Xcode 14, and a guide that tells you to enable it is out of date.}
    \end{column}
  \end{columns}
  \begin{takeaway}{Write XOR execute rules out a JIT in third-party iOS apps.}
...
\begin{frame}[eyebrow=Compilation]{Cross-platform runtimes in 2026}
  \vskip 2mm
  \begin{columns}[T]
    \begin{column}{0.47\textwidth}
      \lead{Flutter}{Dart compiles ahead of time to native ARM for release, so there is no interpreter in production.}
      \vskip 2mm
      \muted{Impeller precompiles its shaders when the engine is built, so no shader is compiled on the user's device. Debug keeps the JIT, which hot reload needs.}
    \end{column}
    \begin{column}{0.47\textwidth}
      \lead{React Native}{Hermes precompiles JavaScript to bytecode at build time, which removes the parse cost at launch.}
      \vskip 2mm
      \muted{The bridgeless architecture, with JSI and Fabric, became the default in React Native 0.76 and the legacy one has since been removed. The serialized-JSON bridge is gone: JavaScript calls C++ directly.}
    \end{column}
  \end{columns}
  ...
  \end{takeaway}
  \note{Correct two claims students bring from older material: React Native no longer serializes JSON over a bridge, and Flutter draws with Impeller on iOS, Android and desktop rather than Skia. Both were true and are not any more.}
\end{frame}

\begin{frame}[eyebrow=Compilation]{Moving work from run time to build time}
  \vskip 2mm
  \begin{grid}[3]
    \cell[\faIcon{paint-brush}]{Shaders}{Impeller precompiles its shaders in the engine, so the first frame with an effect costs what the thousandth costs.}
...
\begin{frame}[eyebrow=Compilation]{Cold start sequence}
  \vskip 2mm
  \begin{columns}[T]
...
% =====================================================================
\divider{Part 2 · Energy}{Energy at the platform level}[Race to idle, thermal throttling, duty cycles]

\begin{frame}[eyebrow=Energy]{Race to idle}
  \vskip 2mm
  \begin{grid}[3]
    \cell[\faIcon{bed}]{Idle is a real state}{A modern SoC has deep low-power states, but only when nothing keeps a core awake.}
  ...
  \begin{takeaway}{Anything that prevents idle costs more than the work it does.}
    That includes a run-time compiler warming up.
  \end{takeaway}
...
\begin{frame}[eyebrow=Energy]{Thermal throttling}
  \vskip 2mm
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      \begin{itemize}
...
\begin{frame}[eyebrow=Energy]{Energy cost of compilation}
  \vskip 2mm
  \begin{grid}[3]
    ...
    \cell[\faCheck]{A build-time decision}{Paid once, on your machine. It costs the user nothing on any launch.}
  \end{grid}
  \vskip 3mm
...
\begin{frame}[embedded,eyebrow=Energy]{Duty cycling at the device level}
  \vskip 1mm
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
    ...
    \end{column}
  \end{columns}
  \begin{takeaway}{Sleep between publishes instead of staying connected.}
  \end{takeaway}
...
\begin{frame}[eyebrow=Energy]{Measurement methodology}
  \vskip 3mm
  \begin{grid}[3]
  ...
  \begin{takeaway}{Report the method with every number.}
  \end{takeaway}
...
% =====================================================================
\divider{Part 3 · Exam preparation}{Exam preparation}[Format, revision plan, example questions]
...
\begin{frame}[eyebrow=Exam]{The exam format}
  ...
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \begin{itemize}
        \item Fifteen multiple-choice questions, 2 points each, 30 points on the paper. The exam is worth 40 of the 100 course points.
...
\begin{frame}[eyebrow=Exam]{Revision topics, Lectures 1 to 7}
  \vskip 2mm
  {\usebeamerfont{small}
...
\begin{frame}[eyebrow=Exam]{Revision topics, Lectures 8 to 14}
  \vskip 2mm
  {\usebeamerfont{small}
  \begin{tabular}{@{}llp{92mm}@{}}
    \toprule
...
\begin{frame}[eyebrow=Exam]{Key numbers for the exam}
  \vskip 2mm
  \begin{grid}[3]
...
\begin{frame}[eyebrow=Exam]{Common mistakes}
  \vskip 2mm
  \begin{grid}[3]
...
\begin{frame}[eyebrow=Exam]{Outdated statements}
  \vskip 2mm
  \begin{grid}[3]
    \cell[\faIcon{paint-brush}]{"Skia draws Flutter"}{Impeller draws on iOS, Android and desktop. Skia is left for the web and old Android.}
    ...
    \cell[\faCloud]{"CodePush for updates"}{The hosted service closed in March 2025. The mainstream successor is Expo EAS Update.}
    \cell[\faLock]{"Enable bitcode"}{Deprecated in Xcode 14, and the App Store no longer accepts it.}
...
\begin{frame}[eyebrow=Exam]{Example question 5: energy}
  ...
  \begin{itemize}
    ...
    \item \textbf{B.} On an OLED panel dark mode is a saving, while on an LCD the backlight is on regardless.
...
\begin{frame}[eyebrow=Project]{Project milestones}
  \vskip 1mm
  {\usebeamerfont{small}
...
\begin{frame}[eyebrow=Project]{Defense rubric}
  \vskip 1mm
  {\usebeamerfont{small}
  ...
  \begin{takeaway}{All five are needed for approval.}
    The 15 points reflect how well each is shown and explained.
  \end{takeaway}
...
\begin{frame}[eyebrow=Project]{Demo guidelines}
  \vskip 2mm
  \begin{grid}[3]
    \cell[\faMobile]{Start cold}{Launch the app in front of the room, not a screen that has been open for an hour.}
    \cell[\faBug]{Break it on purpose}{Pull the cable, turn on airplane mode, kill the broker.}
    ...
    \cell[\faIcon{road}]{Say what is next}{One sentence about a known limitation.}
  \end{grid}
...
\begin{frame}[eyebrow=Project]{Concept presentations}
  ...
  \end{takeaway}
  \note{Confirm the remaining free topics on the sheet before the session ends.}
\end{frame}
...
\begin{frame}[eyebrow=Recap]{Summary}
  \vskip 2mm
  \begin{enumerate}
    \item Every runtime decides when to translate. \muted{Every platform kept moving that moment earlier.}
    \item Energy follows one rule at both scales. \muted{Do the work, then let the hardware sleep.}
    \item Learn the rules with their conditions and the numbers with their units.
  \end{enumerate}
  \vskip 5mm
